...
$.fn.extend({
	devblocksCharts : function(type, options) {
		...
		var drawGauge = function($canvas, options) {
			...
			var color_background_200 = 'rgb(50,50,50)';
			
			if('function' == typeof getComputedStyle) {
				color_background = getComputedStyle(document.documentElement).getPropertyValue('--cerb-color-background');
				color_text = getComputedStyle(document.documentElement).getPropertyValue('--cerb-color-text');
				color_background_200 = getComputedStyle(document.documentElement).getPropertyValue('--cerb-color-background-contrast-200');
			}
			...
			// Accent
			context.beginPath();
			context.strokeStyle = color_background_200;
			context.moveTo(0,0);
			context.lineWidth = 1;
			context.lineTo(-1 * (options.radius * 1.1),0);
			context.stroke();

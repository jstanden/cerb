...
<div style="margin-left:10px;margin-bottom:0.5em;">
	...
	{foreach from=$params.bundle_ids item=bundle_id}
		...
		{if !empty($bundle)}
		<li><input type="hidden" name="{$namePrefix}[bundle_ids][]" value="{$bundle_id}">{$bundle->name} <a data-cerb-link="bundle_remove"><span class="cerb-icons cerb-icon-circle-remove"></span></a></li>
		{/if}
	{/foreach}
	</ul>
</div>

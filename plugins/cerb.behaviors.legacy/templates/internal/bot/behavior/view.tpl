{$view_context = CerberusContexts::CONTEXT_BEHAVIOR}
	...
	<tr>
		...
		{foreach from=$view->view_columns item=header name=headers}
			{if $header == "*_sparkline"}
				{* Sparkline column: a window switcher (24h/7d/30d) in place of a sort link *}
				<th class="no-sort" style="width:170px;">
					...
					<span class="cerb-ui-switcher cerb-ui-switcher--xs" data-cerb-spark-switcher>
						<button type="button" data-value="24h">24h</button>
	...
	<tbody style="cursor:pointer;" class="{if $result.t_is_disabled}disabled{/if}">
		<tr class="{$tableRowClass}">
			...
			<td data-column="label" colspan="{$smarty.foreach.headers.total}">
				...
				{if $result.t_is_private}<span class="cerb-icons cerb-icon-lock" style="margin-top:2px;"></span>{/if}
				...
				<button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.t_id}" data-width="70%"><span class="cerb-icons cerb-icon-new-window"></span></button>
			</td>
		</tr>
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			{elseif $column == "t_bot_id"}
				...
				<td>
					{if isset($bots.$bot_id)}
						...
						<a class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_BOT}" data-context-id="{$bot->id}">{$bot->name}</a>
					{/if}
				</td>
			...
			{elseif in_array($column, ["t_updated_at"])}
				<td>
					{if !empty($result.$column)}
			...
			{elseif $column == "*_sparkline"}
				<td data-column="{$column}" style="width:160px;">
					<div class="cerb-ui-sparkchart" data-cerb-spark="{$result.t_id}" style="min-width:140px;"></div>
				</td>
			{else}
...
$(function() {
	...
	{include file="devblocks:cerberusweb.core::internal/views/sparkline_loader.tpl" spark_module='behavior' spark_action='viewSparklinesJson' spark_view_id=$view->id}
	{/if}

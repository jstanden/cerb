...
<div class="node {$node->node_type}">
	...
	{if is_array($tree[$node_id]) && !empty($tree[$node_id])}
		{foreach from=$tree[$node_id] item=child_id}
			{include file="devblocks:cerb.behaviors.legacy::internal/decisions/simulator/branch.tpl" node_id=$child_id trigger_id=$trigger_id path=$path nodes=$nodes tree=$tree depths=$depths}
		{/foreach}
	{/if}
	</div>
</div>

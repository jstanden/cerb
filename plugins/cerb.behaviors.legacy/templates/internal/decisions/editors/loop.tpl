	...
	<div id="loop{$id}-build">
		<form id="frmDecisionLoop{$id}">
			...
			<div id="divDecisionLoopToolbar{$id}" style="display:none;">
				...
				<button type="button" data-cerb-button="help">Help</button>
		...
		<div class="toolbar">
			<button type="button" data-cerb-button="save"><span class="cerb-icons cerb-icon-circle-ok"></span> {'common.save_changes'|devblocks_translate|capitalize}</button>
			{if isset($id)}<button type="button" data-cerb-button="delete"><span class="cerb-icons cerb-icon-circle-remove"></span> {'common.delete'|devblocks_translate|capitalize}</button>{/if}
		</div>
	</div>
</div>

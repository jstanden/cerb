...
<div class="cerb-ui-form--field">
	<label class="cerb-ui-form--label">{'common.changeset'|devblocks_translate|capitalize}<span class="cerb-ui-form--hint">JSON</span> {include file="devblocks:cerberusweb.core::help/docs_button.tpl" url="https://cerb.ai/docs/records/types/"}</label>
	<textarea name="{$namePrefix}[changeset_json]" class="placeholders" spellcheck="false" rows="5">{$params.changeset_json}</textarea>
</div>

<div class="cerb-ui-form--field">
	<label class="cerb-ui-form--label">Also update records in simulator mode</label>
	<div>

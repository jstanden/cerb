...
class SearchFields_Bot extends DevblocksSearchFields {
		...
		switch($param->field) {
			...
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, CerberusContexts::CONTEXT_BOT, self::getPrimaryKey())))
	...
	static function _getFields() {
		...
		$columns = [
			...
			self::OWNER_CONTEXT => new DevblocksSearchField(self::OWNER_CONTEXT, 'bot', 'owner_context', $translate->_('common.owner_context'), Model_CustomField::TYPE_SINGLE_LINE, false),
			self::OWNER_CONTEXT_ID => new DevblocksSearchField(self::OWNER_CONTEXT_ID, 'bot', 'owner_context_id', $translate->_('common.owner_context_id'), Model_CustomField::TYPE_NUMBER, false),
...
class View_Bot extends C4_AbstractView implements IAbstractView_Subtotals, IAbstractView_QuickSearch {
	...
	function render() {
		...
		$tpl->assign('view_template', 'devblocks:cerb.behaviors.legacy::internal/bot/view.tpl');
...
class Context_Bot extends Extension_DevblocksContext implements IDevblocksContextProfile, IDevblocksContextPeek, IDevblocksContextAutocomplete { // IDevblocksContextImport
	...
	function renderPeekPopup($context_id=0, $view_id='', $edit=false) {
		...
		if(empty($context_id) || $edit) {
			...
			$tpl->assign('view_id', $view_id);
			$tpl->display('devblocks:cerb.behaviors.legacy::internal/bot/peek_edit.tpl');

...
class Event_GetInteractionsForWorker extends Extension_DevblocksEvent {
	...
	static function getByPoint($point) {
		...
		if(null !== ($behaviors = $cache->load($cache_key))) {
			...
			$behaviors = array_filter($behaviors, function($behavior) use ($point) {
				if(!($listen_points = $behavior->event_params['listen_points'] ?? ''))
					return false;
				
				if(!($listen_points = DevblocksPlatform::parseCrlfString($listen_points)) || !is_array($listen_points))
					return false;
	...
	function renderEventParams(?Model_TriggerEvent $trigger=null) {
		...
		$tpl->display('devblocks:cerb.behaviors.legacy::events/interaction/params_interactions_get_for_worker.tpl');
	}
	...
	function renderActionExtension($token, $trigger, $params=array(), $seq=null) {
		...
		switch($token) {
			case 'return_interaction':
				...
				$tpl->display('devblocks:cerb.behaviors.legacy::events/interaction/action_return_interaction.tpl');
				break;
		}

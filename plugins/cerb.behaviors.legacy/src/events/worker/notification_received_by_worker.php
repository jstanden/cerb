...
class Event_NotificationReceivedByWorker extends Extension_DevblocksEvent {
	...
	function generateSampleEventModel(Model_TriggerEvent $trigger, $notification_id=null, $worker_id=null) {
		...
		if(empty($notification_id)) {
			...
			$notification_id = $result[SearchFields_Notification::ID] ?? null;
			$worker_id = $worker->id;
		}
	...
	function setEvent(?Model_DevblocksEvent $event_model=null, ?Model_TriggerEvent $trigger=null) {
		$notification_id = $event_model->params['notification_id'] ?? null;
	...
	function renderActionExtension($token, $trigger, $params=array(), $seq=null) {
		...
		switch($token) {
			...
			case 'send_email_owner':
				...
				$tpl->display('devblocks:cerb.behaviors.legacy::events/notification_received_by_owner/action_send_email_owner.tpl');
				break;
		}
	}

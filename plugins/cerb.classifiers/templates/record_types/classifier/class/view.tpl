{$view_context = CerberusContexts::CONTEXT_CLASSIFIER_CLASS}
	...
	<tbody style="cursor:pointer;">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td>
				...
				<a href="{devblocks_url}c=profiles&type=classifier_class&id={$result.c_id}-{$result.c_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.c_name}</a>
			...
			{elseif $column == "c_classifier_id"}
			<td>
				{if isset($classifiers.{$result.$column})}
					{$classifier = $classifiers.{$result.$column}}
					<a class="cerb-peek-trigger" data-context="{CerberusContexts::CONTEXT_CLASSIFIER}" data-context-id="{$classifier->id}">{$classifier->name}</a>
				{/if}
			</td>

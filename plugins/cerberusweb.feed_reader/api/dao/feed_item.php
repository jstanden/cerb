...
class DAO_FeedItem extends Cerb_ORMHelper {
	...
	static function create($fields) {
		...
		CerberusContexts::checkpointCreations(CerberusContexts::CONTEXT_FEED_ITEM, $id);
		...
		switch($param->field) {
			...
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, CerberusContexts::CONTEXT_FEED_ITEM, self::getPrimaryKey())))
...
class Context_FeedItem extends Extension_DevblocksContext implements IDevblocksContextProfile, IDevblocksContextPeek {
	...
	function getContext($item, &$token_labels, &$token_values, $prefix=null) {
		...
		$token_values['_context'] = Context_FeedItem::ID;
		$token_values['_type'] = Context_FeedItem::URI;
	...
	function renderPeekPopup($context_id=0, $view_id='', $edit=false) {
		...
		if($context_id) {
			if(false == ($model = DAO_FeedItem::get($context_id)))
				DevblocksPlatform::dieWithHttpError(null, 404);
		}
		
		if(!$context_id || $edit) {
			if($model) {
				if(!Context_FeedItem::isWriteableByActor($model, $active_worker))

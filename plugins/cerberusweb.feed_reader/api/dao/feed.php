...
class SearchFields_Feed extends DevblocksSearchFields {
	...
	static function getPrimaryKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_Feed::ID);
	}
		...
		switch($param->field) {
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, CerberusContexts::CONTEXT_FEED, self::getPrimaryKey())))
	...
	function getQuickSearchFields() {
		...
		$fields = array(
			...
			'watchers' => 
				array(
					'type' => DevblocksSearchCriteria::TYPE_VIRTUAL,
					'options' => array('param_key' => DevblocksSearchField::VIRTUAL_WATCHERS, 'match' => DevblocksSearchCriteria::OPTION_TEXT_PARTIAL),
					'examples' => [
...
//		);
	...
	function renderPeekPopup($context_id=0, $view_id='', $edit=false) {
		...
		if($context_id) {
			if(false == ($model = DAO_Feed::get($context_id)))
				DevblocksPlatform::dieWithHttpError(null, 404);
		}
		
		if(!$context_id || $edit) {
			if($model) {
				if(!Context_Feed::isWriteableByActor($model, $active_worker))

	...
	<tbody style="cursor:pointer;">
		<tr class="{$tableRowClass}">
			...
			<td data-column="label" colspan="{$smarty.foreach.headers.total}">
				...
				{if 1 == $result.o_status_id}<span style="color:rgb(0,120,0);font-weight:bold;"><span class="cerb-icons cerb-icon-up-arrow" title="Won" style="line-height:16px;"></span></span>
				{elseif 2 == $result.o_status_id}<span style="color:rgb(150,0,0);font-weight:bold;"><span class="cerb-icons cerb-icon-down-arrow" title="Lost" style="line-height:16px;"></span></span> {/if}
				...
				<button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.o_id}" data-view-id="{$view->id}"><span class="cerb-icons cerb-icon-new-window"></span></button>
			</td>
		</tr>
		<tr class="{$tableRowClass}">
...
<div style="padding-top:5px;">
	...
	<div style="float:left;" id="{$view->id}_actions">
		...
		{if $active_worker->hasPriv("contexts.{$view_context}.update.bulk")}<button data-cerb-worklist-action-bulk="opportunity" type="button" class="action-always-show action-bulkupdate"><span class="cerb-icons cerb-icon-folder"></span> {'common.bulk_update'|devblocks_translate|lower}</button>{/if}

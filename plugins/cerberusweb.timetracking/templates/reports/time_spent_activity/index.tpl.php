...
function drawChart(start, end) {{/literal}
	...
	var cObj = YAHOO.util.Connect.asyncRequest('GET', "{/literal}{devblocks_url}ajax.php?c=reports&a=action&extid=report.timetracking.timespentactivity&extid_a=getTimeSpentActivityChart{/devblocks_url}{literal}&countonly=1&start="+start+"&end="+end, {
		success: function(o) {
			...
			var myXHRDataSource = new YAHOO.util.DataSource("{/literal}{devblocks_url}ajax.php?c=reports&a=action&extid=report.timetracking.timespentactivity&extid_a=getTimeSpentActivityChart{/devblocks_url}{literal}&start="+start+"&end="+end);
...
<h2>Time Spent Per Activity</h2>

<form action="{devblocks_url}{/devblocks_url}" method="POST" id="frmRange" name="frmRange" onsubmit="return false;">
<input type="hidden" name="c" value="reports">
<input type="hidden" name="a" value="action">
<input type="hidden" name="extid" value="report.timetracking.timespentactivity">
<input type="hidden" name="extid_a" value="getTimeSpentActivityReport">

	...
	{/if}
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td>
				...
				<img src="{devblocks_url}c=resource&p=cerberusweb.core&f=images/document.gif{/devblocks_url}" align="top"> <a href="javascript:;" class="ticketLink" style="font-size:12px;" onclick="genericAjaxPanel('c=kb&a=showArticlePeekPanel&id={$result.kb_id}&return={$response_uri|escape:'url'}',null,false,'700px');"><b id="subject_{$result.kb_id}_{$view->id}">{$result.kb_title}</b></a>				
				{/if}
			</td>

	...
	<tr>
		...
		{foreach from=$pages item=m}
			{if !empty($m->manifest->params.menutitle)}
				...
				<td width="1%" nowrap="nowrap" style="padding-left:10px;padding-right:10px;border-right:1px solid rgb(100,135,213);" {if $page->id==$m->id}id="headerMenuSelected"{/if}><a href="{devblocks_url}c={$m->manifest->params.uri}{/devblocks_url}">{$translate->_($m->manifest->params.menutitle)|lower}</a></td>
			{/if}
		{/foreach}

...
{if !empty($results)}
	...
	{foreach from=$results item=result key=article_id}
		...
		<a href="javascript:;" class="ticketLink" style="font-size:12px;" onclick="genericAjaxPanel('c=kb&a=showArticlePeekPanel&id={$result.kb_id}',null,false,'700px');"><b>{$result.kb_title}</b></a><br>
	{/foreach}
	</div>
{else}

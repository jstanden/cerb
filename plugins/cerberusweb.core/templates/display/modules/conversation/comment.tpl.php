...
<div id="displayComment{$comment->id}">
	<div class="block">
		...
		<h3 style="display:inline;"><span style="background-color:rgb(232,242,254);color:rgb(71,133,210);">[comment]</span> <a href="javascript:;" onclick="genericAjaxPanel('c=contacts&a=showAddressPeek&address_id={$comment_address->id}', this, false, '500px', ajax.cbAddressPeek());" title="{$comment_address->email|escape}">{if empty($comment_address->first_name) && empty($comment_address->last_name)}&lt;{$comment_address->email|escape}&gt;{else}{$comment_address->getName()}{/if}</a></h3> &nbsp;  

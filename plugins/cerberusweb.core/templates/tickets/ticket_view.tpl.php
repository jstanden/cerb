...
<table cellpadding="0" cellspacing="0" border="0" class="tableBlue" width="100%" class="tableBg">
	<tr>
		...
		<td nowrap="nowrap" class="tableThBlue" align="right">
			...
			<span style="font-size:12px"> | </span><a href="javascript:;" onclick="genericAjaxGet('{$view->id}_tips','c=tickets&a=showViewAutoAssign&view_id={$view->id}');toggleDiv('{$view->id}_tips','block');" class="tableThLink">{"assign"|lower}</a>
...
<table cellpadding="2" cellspacing="0" border="0" width="100%" class="tableBg" id="{$view->id}_actions">
	...
	<tr>
		<td colspan="2">
			...
			{if $view->id == 'teamwork_my' || $view->id == 'teamwork_my_waiting'}<button type="button" onclick="ajax.viewCloseTickets('{$view->id}',3);"><img src="{devblocks_url}c=resource&p=cerberusweb.core&f=images/flag_white.gif{/devblocks_url}" align="top"> surrender</button>{/if}

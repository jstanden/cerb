	...
	{/if}
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			{elseif $column=="c_worker_id"}
				<td>
					{assign var=c_worker_id value=$result.c_worker_id}
					{if isset($workers.$c_worker_id)}
						{$workers.$c_worker_id->getName()}&nbsp;
					{/if}
				</td>
			{else}
				<td>{$result.$column}</td>
		...
		<tr class="{$tableRowBg}" id="{$rowIdPrefix}_s" onmouseover="toggleClass(this.id,'tableRowHover');toggleClass('{$rowIdPrefix}','tableRowHover');" onmouseout="toggleClass(this.id,'{$tableRowBg}');toggleClass('{$rowIdPrefix}','{$tableRowBg}');" onclick="if(getEventTarget(event)=='TD') checkAll('{$rowIdPrefix}');">
			<td colspan="{math equation="x" x=$smarty.foreach.headers.total}" style="padding:5px;margin-left:5px;">
				<a href="javascript:;" onclick="genericAjaxPanel('c=calls&a=showCallPeek&id={$result.c_id}&view_id={$view->id}',this,false,'550px');" style="color:rgb(75,75,75);font-size:12px;"><b id="subject_{$result.c_id}_{$view->id}">{$result.c_name|escape}</b></a>
				<br>
				{$result.c_content|escape}
			</td>
		</tr>
	{/foreach}
...
<table cellpadding="2" cellspacing="0" border="0" width="100%" class="tableBg" id="{$view->id}_actions">
	...
	<tr>
		<td colspan="2" valign="top">
			{*
			<button type="button" id="btnComplete" onclick="this.form.a.value='viewComplete';genericAjaxPost('viewForm{$view->id}','view{$view->id}','c=calls');"><img src="{devblocks_url}c=resource&p=cerberusweb.core&f=images/check.gif{/devblocks_url}" align="top"> {$translate->_('tasks.complete')|capitalize}</button> 
			<button type="button" id="btnPostpone" onclick="this.form.a.value='viewPostpone';genericAjaxPost('viewForm{$view->id}','view{$view->id}','c=calls');"><img src="{devblocks_url}c=resource&p=cerberusweb.core&f=images/clock.gif{/devblocks_url}" align="top"> {$translate->_('tasks.postpone')|capitalize}</button>
			*}
		</td>
	</tr>
	{/if}

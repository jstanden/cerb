...
class DAO_CallEntry extends Cerb_ORMHelper {
	...
	static function create($fields) {
		...
		CerberusContexts::checkpointCreations(CerberusContexts::CONTEXT_CALL, $id);
	...
	static function getWhereSQL(DevblocksSearchCriteria $param) {
		switch($param->field) {
			...
			case self::VIRTUAL_HAS_FIELDSET:
				return self::_getWhereSQLFromVirtualSearchSqlField($param, CerberusContexts::CONTEXT_CUSTOM_FIELDSET, sprintf('SELECT context_id FROM context_to_custom_fieldset WHERE context = %s AND custom_fieldset_id IN (%s)', Cerb_ORMHelper::qstr(CerberusContexts::CONTEXT_CALL), '%s'), self::getPrimaryKey());
...
class View_CallEntry extends C4_AbstractView implements IAbstractView_Subtotals, IAbstractView_QuickSearch {
	...
	function getParamFromQuickSearchFieldTokens($field, $tokens) {
		switch($field) {
			...
			case 'watchers':
				return DevblocksSearchCriteria::getWatcherParamFromTokens(SearchFields_CallEntry::VIRTUAL_WATCHERS, $tokens);
...
class Context_CallEntry extends Extension_DevblocksContext implements IDevblocksContextProfile, IDevblocksContextPeek, IDevblocksContextImport, IDevblocksContextMerge {
	...
	function renderPeekPopup($context_id=0, $view_id='', $edit=false) {
		...
		if($context_id) {
			if(false == ($model = DAO_CallEntry::get($context_id)))
				DevblocksPlatform::dieWithHttpError(null, 404);
		}
		
		if(!$context_id || $edit) {
			if($model) {
				if(!Context_CallEntry::isWriteableByActor($model, $active_worker))

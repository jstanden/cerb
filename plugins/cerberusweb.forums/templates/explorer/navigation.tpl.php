		...
		<td valign="top">
			<form action="{devblocks_url}{/devblocks_url}" name="formForumThreadActions">
				...
				{if $current_post->is_closed}
					<button type="button" onclick="genericAjaxGet('','c=forums&a=ajaxReopen&id={$current_post->id}');this.innerHTML='Opened!';"><img src="{devblocks_url}c=resource&p=cerberusweb.core&f=images/folder_out.gif{/devblocks_url}" align="top"> {$translate->_('common.reopen')}</button>
				{else}				
					<button type="button" onclick="genericAjaxGet('','c=forums&a=ajaxClose&id={$current_post->id}');this.innerHTML='Closed!';"><img src="{devblocks_url}c=resource&p=cerberusweb.core&f=images/folder_ok.gif{/devblocks_url}" align="top"> {$translate->_('common.close')}</button>
				{/if}
				 &nbsp; 
				{$translate->_('common.assign')}: <select name="worker_id" onchange="genericAjaxGet('','c=forums&a=ajaxAssign&id={$current_post->id}&worker_id='+selectValue(this));">
				...
				</select>{if !empty($active_worker_pos)}<button type="button" onclick="this.form.worker_id.selectedIndex={$active_worker_pos};genericAjaxGet('','c=forums&a=ajaxAssign&id={$current_post->id}&worker_id={$active_worker->id}');">{$translate->_('common.me')}</button>{/if}
				<button type="button" onclick="copyToClipboard('{$current_post->link}');" title="Copy the original URL for this post to the clipboard">{$translate->_('forums.ui.nav.copyurl')}</button>
			</form>
		</td>

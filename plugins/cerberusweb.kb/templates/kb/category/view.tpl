{$view_context = CerberusContexts::CONTEXT_KB_CATEGORY}
	...
	<tbody style="cursor:pointer;">
		<tr class="{$tableRowClass}">
			<td data-column="*_watchers" align="center" nowrap="nowrap" style="padding:5px;">
				{include file="devblocks:cerberusweb.core::internal/watchers/context_follow_button.tpl" context=$view_context context_id=$result.kbc_id}
			</td>
		{foreach from=$view->view_columns item=column name=columns}
			{if DevblocksPlatform::strStartsWith($column, "cf_")}
				{include file="devblocks:cerberusweb.core::internal/custom_fields/view/cell_renderer.tpl"}
			{elseif $column == "kbc_name"}
			<td>
				<input type="checkbox" name="row_id[]" value="{$result.kbc_id}" style="display:none;">
				<a href="{devblocks_url}c=profiles&type=kb_category&id={$result.kbc_id}-{$result.kbc_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.kbc_name}</a>
				<button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.kbc_id}"><span class="cerb-icons cerb-icon-new-window"></span></button>
			</td>
			{elseif in_array($column, ["kbc_parent_id"])}
				<td>
					{if $result.$column}
						{$parent = DAO_KbCategory::get($result.$column)}
						{if $parent}
						<a class="cerb-peek-trigger" data-context="{CerberusContexts::CONTEXT_KB_CATEGORY}" data-context-id="{$parent->id}">{$parent->name}</a>
						{/if}
					{/if}
				</td>
			{elseif in_array($column, ["kbc_updated_at"])}
				<td>

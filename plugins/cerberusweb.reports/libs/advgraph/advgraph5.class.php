...
class graph {
    ...
    private function internalGraph() {
        ...
        if ($this->getProp("showvertscale",true)) {
        	...
        	// [JAS]: Only show precision when it's necessary to go into fractions
        	$prec = ($this->yMax - $this->yMin > $this->getProp("yincpts")) ? 0 : 1;
        	
            for($i=0;$i<$this->getProp("yincpts")+1;$i++) { //Generate scale information for use in the graph image creation
                $vertScaleText[$i] = trim(round((($i/$this->getProp("yincpts"))*($this->yMax-$this->yMin))+$this->yMin,$prec)); // [JAS]: Removed precision

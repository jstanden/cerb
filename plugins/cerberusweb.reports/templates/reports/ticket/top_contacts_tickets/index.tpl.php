...
<button type="button" id="btnSubmit" onclick="genericAjaxPost('frmRange', 'report');drawChart(document.getElementById('start').value, document.getElementById('end').value, document.frmRange.by_address[1].checked);">{$translate->_('common.refresh')|capitalize}</button>
<div id="divCal" style="display:none;position:absolute;z-index:1;"></div>
<br>
<label><input type="radio" name="by_address" value="0" checked onclick="document.getElementById('btnSubmit').click();"></input>{$translate->_('reports.ui.ticket.top_contacts.by_org')}</label>
<label><input type="radio" name="by_address" value="1" onclick="document.getElementById('btnSubmit').click();"></input>{$translate->_('reports.ui.ticket.top_contacts.by_address')}</label>

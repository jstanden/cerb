...
function drawChart(start, end) {{/literal}
	...
	var cObj = YAHOO.util.Connect.asyncRequest('GET', "{/literal}{devblocks_url}ajax.php?c=reports&a=action&extid=report.tickets.open_tickets&extid_a=getOpenTicketsChart{/devblocks_url}{literal}&countonly=1&start="+start+"&end="+end, {
		success: function(o) {
			...
			var myXHRDataSource = new YAHOO.util.DataSource("{/literal}{devblocks_url}ajax.php?c=reports&a=action&extid=report.tickets.open_tickets&extid_a=getOpenTicketsChart{/devblocks_url}{literal}&start="+start+"&end="+end);
...
<input type="hidden" name="extid" value="report.tickets.open_tickets">
<input type="hidden" name="extid_a" value="getOpenTicketsReport">
{$translate->_('reports.ui.created_from')} <input type="text" name="start" id="start" size="10" value="{$start}"><button type="button" onclick="ajax.getDateChooser('divCal',this.form.start);">&nbsp;<img src="{devblocks_url}c=resource&p=cerberusweb.core&f=images/calendar.gif{/devblocks_url}" align="top">&nbsp;</button>

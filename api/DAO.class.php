...
class DAO_Notification extends DevblocksORMHelper {
	...
	public static function create($fields) {
	    $db = DevblocksPlatform::getDatabaseService();
		$id = $db->GenID('notification_seq');
		
		$sql = sprintf("INSERT INTO notification (id,created,worker_id,event_id,read,params) ".
		    "VALUES (%d,%d,0,0,0,'')",
		    $id,
		    time()
		);
	...
	public static function update($id, $fields) {
        self::_update($id, 'notification', $fields);
	}
	...
	public static function getList($ids=array()) {
		...
		$sql = "SELECT id, created, worker_id, event_id, read, params ".
		    "FROM notification n ".
		    (!empty($ids) ? sprintf("WHERE id IN (%s) ", implode(',', $ids)) : " ").
		...
		while(!$rs->EOF) {
		    ...
		    $object->id = intval($rs->fields['id']);
		    $object->worker_id = intval($rs->fields['worker_id']);
		    $object->event_id = intval($rs->fields['event_id']);
		    $object->read = intval($rs->fields['read']);
	...
	public static function markRead($ids,$state=1) {
	    if(!is_array($ids)) $ids = array($ids);
	    if(empty($ids)) return;
	    $db = DevblocksPlatform::getDatabaseService();
	    ...
	    $sql = sprintf("UPDATE notification ".
            "SET read = %d ".
            "WHERE id IN (%s)",
            $state,
            $id_list
	...
	public static function delete($ids) {
	    ...
	    $sql = sprintf("DELETE FROM notification WHERE id IN (%s)", $id_list);
	    $db->Execute($sql) or die(__CLASS__ . '('.__LINE__.')'. ':' . $db->ErrorMsg()); /* @var $rs ADORecordSet */
    ...
    static function search($params, $limit=10, $page=0, $sortBy=null, $sortAsc=null, $withCounts=true) {
        ...
        list($tables,$wheres) = parent::_parseSearchParams($params, SearchFields_Notification::getFields());
		$start = ($page * $limit); // [JAS]: 1-based [TODO] clean up + document
		...
		$sql = sprintf("SELECT ".
			...
			"FROM notification n ",
			    ...
			    SearchFields_Notification::EVENT_ID,
			    SearchFields_Notification::READ,
			    SearchFields_Notification::PARAMS
			).
...
//			(isset($tables['ra']) ? "INNER JOIN requester r ON (r.ticket_id=t.id)" : " ").
		...
		while(!$rs->EOF) {
			...
			$ticket_id = intval($rs->fields[SearchFields_Notification::ID]);
			$results[$ticket_id] = $result;
			$rs->MoveNext();
		}
...
class SearchFields_Notification implements IDevblocksSearchFields {
	...
	static function getFields() {
		return array(
			SearchFields_Notification::ID => new DevblocksSearchField(SearchFields_Notification::ID, 'n', 'id'),
			SearchFields_Notification::CREATED => new DevblocksSearchField(SearchFields_Notification::CREATED, 'n', 'created'),
			SearchFields_Notification::WORKER_ID => new DevblocksSearchField(SearchFields_Notification::WORKER_ID, 'n', 'worker_id'),
			SearchFields_Notification::EVENT_ID => new DevblocksSearchField(SearchFields_Notification::EVENT_ID, 'n', 'event_Id'),
			SearchFields_Notification::PARAMS => new DevblocksSearchField(SearchFields_Notification::PARAMS, 'n', 'params'),
		);
	}
};	

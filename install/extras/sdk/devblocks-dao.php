		...
		switch($param->field) {
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, Context_<?php echo $class_name; ?>::ID, self::getPrimaryKey())))
...
{if !empty($model->id)}
	{include file="devblocks:cerberusweb.core::internal/peek/delete_confirm.tpl" noun="<?php echo mb_strtolower($object_name); ?>"}
{/if}
...
<div class="buttons" style="margin-top:10px;">
	...
	{else}
		...
		{*<button type="button" class="create-continue"><span class="cerb-icons cerb-icon-circle-arrow-right"></span> {'common.create_and_continue'|devblocks_translate|capitalize}</button>*}
	{/if}
</div>
	...
	<tbody style="cursor:pointer;">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td>
				...
				<button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.<?php echo $field_prefix; ?>_id}"><span class="cerb-icons cerb-icon-new-window"></span></button>
			</td>

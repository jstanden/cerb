...
<fieldset class="peek">
	...
	<div style="margin-top:5px;">
		<button type="button" class="cerb-search-trigger" data-context="{Context_ProjectBoardColumn::ID}" data-query="board.id:{$dict->id}"><div class="badge-count">{$activity_counts.board_columns|default:0}</div> {'projects.common.board.columns'|devblocks_translate|capitalize}</button>
	</div>
	...
	$popup.one('popup_open',function(event,ui) {
		...
		$popup.find('.cerb-peek-profile').click(function(e) {
			if(e.shiftKey || e.metaKey) {
				window.open('{devblocks_url}c=profiles&type=project_board&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}', '_blank', 'noopener');

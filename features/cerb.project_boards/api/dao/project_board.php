...
class DAO_ProjectBoard extends Cerb_ORMHelper {
	...
	static function autocomplete($term, $as='models') {
		...
		switch($as) {
			...
			default:
				return DAO_ProjectBoard::getIds(array_keys($objects));
		}
	}
...
class SearchFields_ProjectBoard extends DevblocksSearchFields {
		...
		switch($param->field) {
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, Context_ProjectBoard::ID, self::getPrimaryKey())))
	...
	static function _getFields() {
		...
		$columns = [
			...
			self::OWNER_CONTEXT => new DevblocksSearchField(self::OWNER_CONTEXT, 'project_board', 'owner_context', null, Model_CustomField::TYPE_SINGLE_LINE, true),
			self::OWNER_CONTEXT_ID => new DevblocksSearchField(self::OWNER_CONTEXT_ID, 'project_board', 'owner_context_id', null, Model_CustomField::TYPE_NUMBER, true),
...
class Context_ProjectBoard extends Extension_DevblocksContext implements IDevblocksContextProfile, IDevblocksContextPeek, IDevblocksContextAutocomplete, IDevblocksContextWorkflow { // IDevblocksContextImport
	...
	function autocomplete($term, $query=null) {
		...
		$results = DAO_ProjectBoard::autocomplete($term);
	...
	function getContext($project_board, &$token_labels, &$token_values, $prefix=null) {
		...
		$token_labels = array(
			...
			'cards_kata' => $prefix.$translate->_('common.cards'),

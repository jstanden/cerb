...
<table cellpadding="1" cellspacing="0" border="0" width="100%" class="tableRowBg">
	...
	<tr class="tableTh">
		...
		{foreach from=$view->view_columns item=header name=headers}
			...
			<th nowrap="nowrap">
			{if $header=="x"}<a href="javascript:;" onclick="genericAjaxGet('view{$view->id}','c=internal&a=viewSortBy&id={$view->id}&sortBy=a_id');">{$translate->_('common.id')|capitalize}</a>
	...
	{assign var=rowIdPrefix value="row_"|cat:$view->id|cat:"_"|cat:$result.wmf_id}
	...
	{assign var=worker_id value=$result.wmf_worker_id}
		...
		<tr class="{$tableRowBg}" id="{$rowIdPrefix}_s" onmouseover="toggleClass(this.id,'tableRowHover');toggleClass('{$rowIdPrefix}','tableRowHover');" onmouseout="toggleClass(this.id,'{$tableRowBg}');toggleClass('{$rowIdPrefix}','{$tableRowBg}');" onclick="if(getEventTarget(event)=='TD'||getEventTarget(event)=='DIV') checkAll('{$rowIdPrefix}_s');">
			<td align="center" rowspan="2"><input type="checkbox" name="row_id[]" value="{$result.wmf_id}"></td>
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			{elseif $column=="wmf_created"}
			<td title="{$result.wmf_created|devblocks_date}">{$result.wmf_created|devblocks_prettytime}&nbsp;</td>

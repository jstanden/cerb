...
$(function() {
	...
	$popup.one('popup_open',function(event,ui) {
		$popup.dialog('option','title', "{'crm.common.opportunity'|devblocks_translate|capitalize|escape:'javascript' nofilter}");
		...
		// Edit button
		{if $active_worker->hasPriv('contexts.cerberusweb.contexts.opportunity.update')}
		...
		$popup.find('.cerb-peek-profile').click(function(e) {
			if(e.shiftKey || e.metaKey) {
				window.open('{devblocks_url}c=profiles&type=opportunity&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}', '_blank', 'noopener');
				
			} else {
				document.location='{devblocks_url}c=profiles&type=opportunity&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}';
			}
		});

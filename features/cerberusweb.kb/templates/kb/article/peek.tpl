...
<fieldset class="peek">
	...
	<div style="margin-top:5px;">
		<button type="button" class="cerb-search-trigger" data-context="{CerberusContexts::CONTEXT_KB_CATEGORY}" data-query="article.id:{$dict->id}"><div class="badge-count">{$activity_counts.categories|default:0}</div> {'common.categories'|devblocks_translate|capitalize}</button>
	</div>
	...
	$popup.one('popup_open',function(event,ui) {
		...
		$popup.find('.cerb-peek-profile').click(function(e) {
			if(e.shiftKey || e.metaKey) {
				window.open('{devblocks_url}c=profiles&type=kb&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}', '_blank', 'noopener');
				
			} else {
				document.location='{devblocks_url}c=profiles&type=kb&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}';
			}
		});

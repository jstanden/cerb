	...
	<td width="1%" nowrap="nowrap" valign="top" style="padding-right:5px;">
		<form name="compose" enctype="multipart/form-data" method="post" action="{devblocks_url}{/devblocks_url}">
			...
			{if $root_id}
				...
				{if $parent_id}
					{if $active_worker->hasPriv('core.kb.categories.modify')}<button type="button" onclick="genericAjaxPopup('peek','c=kb.ajax&a=showKbCategoryEditPanel&id={$root_id}&return={$response_uri|escape:'url'}',null,false,'500');"><span class="cerb-sprite sprite-folder_gear"></span> Edit Category</button>{/if}
				{else}
					{if $active_worker->hasPriv('core.kb.topics.modify')}<button type="button" onclick="genericAjaxPopup('peek','c=kb.ajax&a=showTopicEditPanel&id={$root_id}&return={$response_uri|escape:'url'}',null,false,'500');"><span class="cerb-sprite sprite-folder_gear"></span> Edit Topic</button>{/if}
				{/if}
				{if $active_worker->hasPriv('core.kb.categories.modify')}<button type="button" onclick="genericAjaxPopup('peek','c=kb.ajax&a=showKbCategoryEditPanel&id=0&root_id={$root_id}&return={$response_uri|escape:'url'}',null,false,'500');"><span class="cerb-sprite sprite-folder_add"></span> Add Subcategory</button>{/if}
			{else}
				{if $active_worker->hasPriv('core.kb.topics.modify')}<button type="button" onclick="genericAjaxPopup('peek','c=kb.ajax&a=showTopicEditPanel&id=0&return={$response_uri|escape:'url'}',null,false,'500');"><span class="cerb-sprite sprite-folder_add"></span> Add Topic</button>{/if}
			{/if}

...
{if $active_worker->hasPriv('contexts.cerberusweb.contexts.kb_article.update')}<button type="button" onclick="genericAjaxPopup('peek','c=kb.ajax&a=showArticleEditPanel&id={$article->id}&view_id={$view_id}',null,false,'725');"><span class="glyphicons glyphicons-edit"></span> {'common.edit'|devblocks_translate|capitalize}</button>{/if}
</form>

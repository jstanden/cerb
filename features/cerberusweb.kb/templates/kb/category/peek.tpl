...
{$is_writeable = Context_KbCategory::isWriteableByActor($dict, $active_worker)}
...
<div id="{$div_id}">
	...
	<div style="float:left;">
		...
		<div style="font-weight:bold;">
			...
			<a href="javascript:;" class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_KB_CATEGORY}" data-context-id="{$breadcrumb->id}">{$breadcrumb->name}</a> &raquo;
			{/if}
			{/foreach}
		</div>
		{/if}
	...
	$popup.one('popup_open',function(event,ui) {
		...
		$popup.find('.cerb-peek-profile').click(function(e) {
			if(e.shiftKey || e.metaKey) {
				window.open('{devblocks_url}c=profiles&type=kb_category&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}', '_blank', 'noopener');
				
			} else {
				document.location='{devblocks_url}c=profiles&type=kb_category&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}';
			}
		});

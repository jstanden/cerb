...
<table cellpadding="1" cellspacing="0" border="0" width="100%" class="tableRowBg">
	...
	{assign var=rowIdPrefix value="row_"|cat:$view->id|cat:"_"|cat:$result.ct_id}
	...
	{/if}
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td>
				<a href="{devblocks_url}c=community&portal={$result.ct_code}{/devblocks_url}">{if !empty($result.ct_name)}{$result.ct_name|escape}{elseif isset($tool_extensions.$extid)}{$tool_extensions.$extid->name|escape}{else}(no name){/if}</a>&nbsp;
			</td>

<fieldset class="peek">
<legend>{$translate->_('timetracking.ui.reports.time_spent_activity')}</legend>
...
{if !empty($data)}
	...
	<div>
		...
		<tr>
			<td></td>
			{foreach from=$data key=activity_id item=plots}
				<td style="font-weight:bold;" nowrap="nowrap">{$activities.$activity_id}</td>
			{/foreach}
		</tr>
		...
		<tr>
			...
			{foreach from=$data item=plots key=activity_id}
				...
				{if isset($plots.$tick)}
					{$plots.$tick}
					{$sums[$activity_id] = intval($sums.$activity_id) + $plots.$tick}
				{/if}
				</td>
			{/foreach}
		</tr>
		...
		<tr>
			...
			{foreach from=$sums key=activity_id item=sum}
				<td align="center">
					<b>{$sum}</b>
				</td>
			{/foreach}

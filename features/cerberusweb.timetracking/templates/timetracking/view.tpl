...
<table cellpadding="0" cellspacing="0" border="0" class="worklist" width="100%" {if array_key_exists('header_color', $view->options) && $view->options.header_color}style="background-color:{$view->options.header_color};"{/if}>
	<tr>
		...
		<td nowrap="nowrap" align="right" class="title-toolbar">
			{if $active_worker->hasPriv("contexts.{$view_context}.create")}<a title="{'common.add'|devblocks_translate|capitalize}" class="minimal cerb-peek-trigger" data-context="{$view_context}" data-context-id="0" data-edit="true"><span class="cerb-icons cerb-icon-circle-plus"></span></a>{/if}
	...
	<tbody style="cursor:pointer;">
		<tr class="{$tableRowClass}">
			...
			<td data-column="label" colspan="{$smarty.foreach.headers.total}">
				...
				{if $result.tt_is_closed}<span class="cerb-icons cerb-icon-circle-ok" style="font-size:16px;color:rgb(80,80,80);"></span>{/if}
				...
				<button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.tt_id}"><span class="cerb-icons cerb-icon-new-window"></span></button>
			</td>
		</tr>
		<tr class="{$tableRowClass}">
...
<div style="padding-top:5px;">
	...
	<div style="float:left;" id="{$view->id}_actions">
		...
		{if $active_worker->hasPriv("contexts.{$view_context}.update.bulk")}<button data-cerb-worklist-action-bulk="time_tracking" type="button" class="action-always-show action-bulkupdate"><span class="cerb-icons cerb-icon-folder"></span> {'common.bulk_update'|devblocks_translate|lower}</button>{/if}
		{if $active_worker->hasPriv("contexts.{$view_context}.update")}<button type="button" class="action-close"><span class="cerb-icons cerb-icon-circle-ok" style="color:rgb(0,180,0);"></span> {'common.close'|devblocks_translate|lower}</button>{/if}
	</div>
</div>
{/if}
...
$(function() {
	...
	$frm.find('.action-explore').on('click', function() {
		$frm.find('[name=explore_from]').val($frm.find('tbody input:checkbox:checked:first').val());
		$frm.submit();
	});

	$frm.find('.action-close').on('click', function() {
		var formData = new FormData($frm[0]);
		formData.set('c', 'profiles');
		formData.set('a', 'invoke');
		formData.set('module', 'time_tracking');
		formData.set('action', 'viewMarkClosed');
		genericAjaxPost(formData, 'view{$view->id}', '');
	});

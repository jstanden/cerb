...
//					'worker_id' => $active_worker->id,
					'total' => $total,
					'return_url' => isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : $url_writer->writeNoProxy('c=search&type=timetracking_activity', true),
					'toolbar_extension_id' => 'cerberusweb.contexts.timetracking.activity.explore.toolbar',
				);
				$models[] = $model;
			...
			foreach($results as $opp_id => $row) {
				...
				$url = $url_writer->writeNoProxy(sprintf("c=profiles&type=timetracking_activity&id=%d-%s", $row[SearchFields_TimeTrackingActivity::ID], DevblocksPlatform::strToPermalink($row[SearchFields_TimeTrackingActivity::NAME])), true);
				
				$model = new Model_ExplorerSet();
				$model->hash = $hash;
				$model->pos = $pos++;
				$model->params = array(
					'id' => $row[SearchFields_TimeTrackingActivity::ID],
					'url' => $url,
				);
				$models[] = $model;
			}

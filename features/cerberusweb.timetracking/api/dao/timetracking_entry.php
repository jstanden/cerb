...
class DAO_TimeTrackingEntry extends Cerb_ORMHelper {
	...
	static function processUpdateEvents($ids, $change_fields) {
		...
		if(!($before_models = CerberusContexts::getCheckpoints(CerberusContexts::CONTEXT_TIMETRACKING, $ids)))
			return;
		
		if(!($models = DAO_TimeTrackingEntry::getIds($ids)))
			return;
		...
		switch($param->field) {
			...
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, CerberusContexts::CONTEXT_TIMETRACKING, self::getPrimaryKey())))

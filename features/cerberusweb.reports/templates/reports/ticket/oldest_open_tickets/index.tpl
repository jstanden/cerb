	...
	{foreach from=$oldest_tickets key=group_id item=group_tickets}
		{if !empty($group_tickets)}
			...
			{foreach from=$group_tickets item=ticket_entry}
				<tr>
					<td style="padding-left:10px;padding-right:20px;"><a href="{devblocks_url}c=profiles&type=ticket&id={$ticket_entry->mask}{/devblocks_url}">{$ticket_entry->mask}</a></td>
					<td><a href="{devblocks_url}c=profiles&type=ticket&id={$ticket_entry->mask}{/devblocks_url}">{$ticket_entry->subject}</a></td>

...
<form action="{devblocks_url}c=reports&report=report.tickets.new_tickets{/devblocks_url}" method="POST" id="frmRange">
	...
	$(function() {
		...
		var options = {
			...
			yaxis: {
				ticks: [
					{foreach from=$data item=row key=iter name=iters}
					[{$iter},"<b>{$groups.{$row.group_id}->name|escape}</b>"]{if !$smarty.foreach.iters.last},{/if}
					{/foreach}
				]
			}
		} ;

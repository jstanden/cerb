...
class SearchFields_Classifier extends DevblocksSearchFields {
	...
	static function getPrimaryKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_Classifier::ID);
	}
	
	static function getUpdatedKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_Classifier::UPDATED_AT);
	}
		...
		switch($param->field) {
			...
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, CerberusContexts::CONTEXT_CLASSIFIER, self::getPrimaryKey())))

			...
			switch($param_key) {
				case SearchFields_FeedItem::VIRTUAL_WORKERS:
					...
					if(empty($param->value)) { // empty
						$join_sql .= "LEFT JOIN context_link AS context_owner ON (context_owner.from_context = 'cerberusweb.contexts.feed.item' AND context_owner.from_context_id = feed_item.id AND context_owner.to_context = 'cerberusweb.contexts.worker') ";
						$where_sql .= "AND context_owner.to_context_id IS NULL ";
					} else {
						$join_sql .= sprintf("INNER JOIN context_link AS context_owner ON (context_owner.from_context = 'cerberusweb.contexts.feed.item' AND context_owner.from_context_id = feed_item.id AND context_owner.to_context = 'cerberusweb.contexts.worker' AND context_owner.to_context_id IN (%s)) ",
							implode(',', $param->value)
						);
					}
					break;
			...
			if(isset($do['owner']) && is_array($do['owner'])) {
				...
				foreach($batch_ids as $batch_id) {
					if(isset($owner_params['add']) && is_array($owner_params['add']))
						CerberusContexts::addWorkers('cerberusweb.contexts.feed.item', $batch_id, $owner_params['add']);
					if(isset($owner_params['remove']) && is_array($owner_params['remove']))
						CerberusContexts::removeWorkers('cerberusweb.contexts.feed.item', $batch_id, $owner_params['remove']);
				}
			}

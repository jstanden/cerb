{$page_context = CerberusContexts::CONTEXT_WEBAPI_CREDENTIAL}
{$page_context_id = $webapi_credentials->id}
{$is_writeable = Context_WebApiCredentials::isWriteableByActor($webapi_credentials, $active_worker)}

<h1>{$webapi_credentials->name}</h1>
...
<div class="cerb-profile-toolbar">
	<form class="toolbar" action="{devblocks_url}{/devblocks_url}" onsubmit="return false;" style="margin-bottom:5px;">
		...
		<button type="button" id="btnDisplayWebApiCredentialsEdit" title="{'common.edit'|devblocks_translate|capitalize} (E)" class="cerb-peek-trigger" data-context="{$page_context}" data-context-id="{$page_context_id}" data-edit="true"><span class="glyphicons glyphicons-cogwheel"></span></button>
		{/if}
	</form>
...
<fieldset class="properties">
	<legend>{'webapi.common.api_credentials'|devblocks_translate|capitalize}</legend>
...
$(function() {
	...
	tabOptions.active = Devblocks.getjQueryUiTabSelected('webapi_credentialsTabs');

	var tabs = $("#webapi_credentialsTabs").tabs(tabOptions);

	$('#btnProfileCard').cerbPeekTrigger();
	
	// Edit
	{if $is_writeable && $active_worker->hasPriv("contexts.{$page_context}.update")}
	$('#btnDisplayWebApiCredentialsEdit')
		.cerbPeekTrigger()
	...
	$(document).keypress(function(event) {
		...
		switch(event.which) {
			...
			case 58:  // (0) tab cycle
				try {
					idx = event.which-49;
					$tabs = $("#webapi_credentialsTabs").tabs();
					$tabs.tabs('option', 'active', idx);
				} catch(ex) { }
				break;
			case 101:  // (E) edit
				try {
					$('#btnDisplayWebApiCredentialsEdit').click();
				} catch(ex) { }
				break;
			case 109:  // (M) macros
				try {

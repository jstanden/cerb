...
class Page_Login extends CerberusPageExtension {
	...
	private function _routeAuthenticate() {
		...
		// If no email/password, fail without further work
		if(!$email || !$password) {
			$query = ['error' => 'auth.failed'];
			DevblocksPlatform::redirect(new DevblocksHttpRequest(['login'], $query));
		}

		// If the email doesn't match a worker, run a dummy bcrypt verify so the
		// response time doesn't distinguish "no such email" from "wrong password"
		// (user enumeration via timing). The hash below was generated by
		// password_hash() at PASSWORD_DEFAULT cost; its plaintext is random and
		// was discarded, so password_verify() always returns false.
		if(!($unauthenticated_worker = DAO_Worker::getByEmail($email))) {
			password_verify($password, '$2y$12$jJTYvxXn40N9M5RxfCTncOZWELvy6.1gu.QqTAygrBOWCofB3HEpq');

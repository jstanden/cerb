		...
		do {
			...
			if(0==$view->renderPage) {
				...
				$model->params = array(
					...
					'created' => time(),
					'total' => $total,
					'return_url' => isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : $url_writer->writeNoProxy('c=search&type=gpg_private_key', true),
				);
				$models[] = $model;
			...
			if(is_array($results))
				foreach($results as $opp_id => $row) {
					...
					$url = $url_writer->writeNoProxy(sprintf("c=profiles&type=gpg_private_key&id=%d-%s", $row[SearchFields_GpgPrivateKey::ID], DevblocksPlatform::strToPermalink($row[SearchFields_GpgPrivateKey::NAME])), true);
					
					$model = new Model_ExplorerSet();
					$model->hash = $hash;
					$model->pos = $pos++;
					$model->params = array(
						'id' => $row[SearchFields_GpgPrivateKey::ID],
						'url' => $url,
					);
					$models[] = $model;
				}

		...
		do {
			...
			if(0==$view->renderPage) {
				...
				$model->params = array(
					...
					'total' => $total,
					'return_url' => isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : $url_writer->writeNoProxy('c=search&type=queue', true),
				);
				$models[] = $model;
			...
			if(is_array($results))
				foreach($results as $opp_id => $row) {
					...
					$url = $url_writer->writeNoProxy(sprintf("c=profiles&type=queue&id=%d-%s", $row[SearchFields_Queue::ID], DevblocksPlatform::strToPermalink($row[SearchFields_Queue::NAME])), true);
					
					$model = new Model_ExplorerSet();
					$model->hash = $hash;
					$model->pos = $pos++;
					$model->params = array(
						'id' => $row[SearchFields_Queue::ID],
						'url' => $url,
					);
					$models[] = $model;
				}

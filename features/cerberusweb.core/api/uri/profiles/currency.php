...
class PageSection_ProfilesCurrency extends Extension_PageSection {
	function render() {
		...
		$labels = $values = [];
		CerberusContexts::getContext($context, $currency, $labels, $values, '', true, false);
		...
		@$values = array_shift(DAO_CustomFieldValue::getValuesByContextIds($context, $currency->id)) or [];
		...
		$properties_custom_fieldsets = Page_Profiles::getProfilePropertiesCustomFieldsets($context, $currency->id, $values);

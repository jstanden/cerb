<?php
class ProfileWidget_ChartKata extends Extension_ProfileWidget {
	const ID = 'cerb.profile.tab.widget.chart.kata';
	...
	function render(Model_ProfileWidget $model, $context, $context_id) {
		...
		$tpl->assign('chart_id', sprintf('profileWidget%d', $model->id));
	 ...
	 */
	private function _getChartJson(Model_ProfileWidget $model, $context, $context_id, string $chart_kata, string $datasets_kata) : array {
		$active_worker = CerberusApplication::getActiveWorker();
		$worker_id = $active_worker?->id ?? 0;

		// Resolve {{record_*}} placeholders against the profile's record
	...
	function renderConfig(Model_ProfileWidget $model) {
		...
		$tpl->display('devblocks:cerberusweb.core::internal/profiles/widgets/chart/kata/config.tpl');
	}
	...
	private function _widgetConfig_previewDataset(Model_ProfileWidget $model) {
	...
	private function _widgetConfig_previewChart(Model_ProfileWidget $model) {
		$params = DevblocksPlatform::importGPC($_POST['params'] ?? [], 'array', []);
	...
	// Placeholder seed for the testers; the record (`record_*`) is supplied by
	// the placeholder simulator since there's no live record in the config popup.
	private function _getTesterInitialState(Model_ProfileWidget $model) : array {
		$active_worker = CerberusApplication::getActiveWorker();
		$worker_id = $active_worker?->id ?? 0;

...
class SearchFields_WorkspacePage extends DevblocksSearchFields {
	...
	static function getPrimaryKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_WorkspacePage::ID);
	}
	
	static function getUpdatedKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_WorkspacePage::UPDATED_AT);
	}
		...
		switch($param->field) {
			...
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, CerberusContexts::CONTEXT_WORKSPACE_PAGE, self::getPrimaryKey())))
	...
	static function _getFields() {
		...
		$columns = [
			self::ID => new DevblocksSearchField(self::ID, 'workspace_page', 'id', $translate->_('common.id'), Model_CustomField::TYPE_NUMBER, true),
			...
			self::OWNER_CONTEXT => new DevblocksSearchField(self::OWNER_CONTEXT, 'workspace_page', 'owner_context', null, Model_CustomField::TYPE_SINGLE_LINE, false),
			self::OWNER_CONTEXT_ID => new DevblocksSearchField(self::OWNER_CONTEXT_ID, 'workspace_page', 'owner_context_id', null, Model_CustomField::TYPE_NUMBER, false),
			self::EXTENSION_ID => new DevblocksSearchField(self::EXTENSION_ID, 'workspace_page', 'extension_id', $translate->_('common.type'), Model_CustomField::TYPE_SINGLE_LINE, true),
...
class Model_WorkspacePage extends DevblocksRecordModel {
	...
	function getOwnerMeta() {
		if(!($page_owner_context_ext = Extension_DevblocksContext::get($this->owner_context)))
			return null;
...
class View_WorkspacePage extends C4_AbstractView implements IAbstractView_QuickSearch, IAbstractView_Subtotals {
	...
	protected function _getData() {
		return DAO_WorkspacePage::search(
			$this->view_columns,
			$this->getParams(),
			$this->renderLimit,
			$this->renderPage,
...
class Context_WorkspacePage extends Extension_DevblocksContext implements IDevblocksContextProfile, IDevblocksContextPeek, IDevblocksContextWorkflow {
	...
	function getContext($page, &$token_labels, &$token_values, $prefix=null) {
		...
		$token_values['_context'] = Context_WorkspacePage::ID;
		$token_values['_type'] = Context_WorkspacePage::URI;
	...
	function renderPeekPopup($context_id=0, $view_id='', $edit=false) {
		...
		if($context_id) {
			if(false == ($model = DAO_WorkspacePage::get($context_id)))
				DevblocksPlatform::dieWithHttpError(null, 404);
		}
		
		if(!$context_id || $edit) {
			if($model) {
				if(!Context_WorkspacePage::isWriteableByActor($model, $active_worker))
	...
	function workflowExport(array $ids, DevblocksWorkflowExportModel $export_model, bool $include_children = false): array {
		...
		$context_workspace_tab = Extension_DevblocksContext::getByAlias('workspace_tab', true);
		
		$models = DAO_WorkspacePage::getIds($ids);
		...
		foreach($models as $model) {
			...
			if($include_children) {
				$workspace_tabs = $model->getTabs();
				$custom_field_kata = $context_workspace_tab->workflowExport(array_keys($workspace_tabs), $export_model, $include_children);

...
class SearchFields_ContextBulkUpdate extends DevblocksSearchFields {
	...
	static function getTableName() : string {
		return 'context_bulk_update';
	}
	
	static function getPrimaryKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_ContextBulkUpdate::ID);
	}
	
	static function getUpdatedKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_ContextBulkUpdate::CREATED_AT);
	}
		...
		switch($param->field) {
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, 'cerberusweb.contexts.context.bulk.update', self::getPrimaryKey())))

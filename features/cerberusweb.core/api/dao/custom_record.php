...
class SearchFields_CustomRecord extends DevblocksSearchFields {
	...
	static function getPrimaryKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_CustomRecord::ID);
	}
	
	static function getUpdatedKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_CustomRecord::UPDATED_AT);
	}
		...
		switch($param->field) {
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, CerberusContexts::CONTEXT_CUSTOM_RECORD, self::getPrimaryKey())))
...
class View_CustomRecord extends C4_AbstractView implements IAbstractView_Subtotals, IAbstractView_QuickSearch {
	...
	protected function _getData() {
		return DAO_CustomRecord::search(
			$this->view_columns,
			$this->getParams(),
			$this->renderLimit,
			$this->renderPage,
...
class Context_CustomRecord extends Extension_DevblocksContext implements IDevblocksContextProfile, IDevblocksContextPeek, IDevblocksContextWorkflow { // IDevblocksContextImport
	...
	function renderPeekPopup($context_id=0, $view_id='', $edit=false) {
		...
		if($context_id) {
			if(!($model = DAO_CustomRecord::get($context_id)))
	...
	function workflowExport(array $ids, DevblocksWorkflowExportModel $export_model, bool $include_children = false) : array {
		...
		$models = DAO_CustomRecord::getIds($ids);
		...
		foreach($models as $model) {
			...
			if($include_children) {
				$custom_fields = DAO_CustomField::getByContext(sprintf('contexts.custom_record.%d', $model->id));

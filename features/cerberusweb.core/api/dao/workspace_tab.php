...
class SearchFields_WorkspaceTab extends DevblocksSearchFields {
	...
	static function getPrimaryKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_WorkspaceTab::ID);
	}
	
	static function getUpdatedKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_WorkspaceTab::UPDATED_AT);
	}
		...
		switch($param->field) {
			...
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, CerberusContexts::CONTEXT_WORKSPACE_TAB, self::getPrimaryKey())))
...
class Context_WorkspaceTab extends Extension_DevblocksContext implements IDevblocksContextPeek, IDevblocksContextProfile, IDevblocksContextWorkflow {
	...
	function getKeyToDaoFieldMap() {
		return [
			...
			'options_kata' => DAO_WorkspaceTab::OPTIONS_KATA,

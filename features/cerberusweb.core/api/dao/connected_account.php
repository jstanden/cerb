...
class DAO_ConnectedAccount extends Cerb_ORMHelper {
	...
	public static function getSearchQueryComponents($columns, $params, $sortBy=null, $sortAsc=null) {
		...
		array_walk_recursive(
			$params,
			array('DAO_ConnectedAccount', '_translateVirtualParameters'),
			$args
		);
	...
	private static function _translateVirtualParameters($param, $key, &$args) {
		...
		$from_context = CerberusContexts::CONTEXT_CONNECTED_ACCOUNT;
		$from_index = 'connected_account.id';
...
class View_ConnectedAccount extends C4_AbstractView implements IAbstractView_Subtotals, IAbstractView_QuickSearch {
	...
	function renderCriteria($field) {
		...
		switch($field) {
			...
			case SearchFields_ConnectedAccount::VIRTUAL_HAS_FIELDSET:
				$this->_renderCriteriaHasFieldset($tpl, CerberusContexts::CONTEXT_CONNECTED_ACCOUNT);
				break;

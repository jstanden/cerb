<?php
class DAO_MailRoutingRule extends Cerb_ORMHelper {
	...
	static function create($fields) {
		...
		if(!array_key_exists(DAO_MailRoutingRule::CREATED_AT, $fields))
			$fields[DAO_MailRoutingRule::CREATED_AT] = time();
		
		$sql = "INSERT INTO mail_routing_rule () VALUES ()";
		$db->ExecuteMaster($sql);
		$id = $db->LastInsertId();
		
		CerberusContexts::checkpointCreations(Context_MailRoutingRule::ID, $id);
	...
	static function update($ids, $fields, $check_deltas=true) {
		...
		while($batch_ids = array_shift($chunks)) {
			...
			// Make changes
			parent::_update($batch_ids, 'mail_routing_rule', $fields);
	...
	static function updateWhere($fields, $where) {
		self::clearCache();
		parent::_updateWhere('mail_routing_rule', $fields, $where);
	}
	...
	static public function onBeforeUpdateByActor($actor, &$fields, $id=null, &$error=null) {
		...
		if(array_key_exists(self::ROUTING_KATA, $fields)) {
			$kata = DevblocksPlatform::services()->kata();
			
			if(false === $kata->validate(
				$fields[self::ROUTING_KATA],
				CerberusApplication::kataSchemas()->bucketRouting(),
				$error
			)) {
				$error = 'Routing: ' . $error;
				return false;
	...
	static function getAll(bool $nocache=false) : array {
		...
		if($nocache || null === ($objects = $cache->load(self::_CACHE_ALL))) {
			$objects = self::getWhere(null, DAO_MailRoutingRule::PRIORITY, true, null, DevblocksORMHelper::OPT_GET_MASTER_ONLY);
	 ...
	 * @param integer $id
	 */
	static function get($id) : ?Model_MailRoutingRule {
		if(empty($id))
			return null;
	...
	static private function _getObjectsFromResult($rs) : array {
		...
		while($row = mysqli_fetch_assoc($rs)) {
			$object = new Model_MailRoutingRule();
	...
	static function random() {
		return self::_getRandom('mail_routing_rule');
	}
	...
	static function delete($ids) {
		...
		$db->ExecuteMaster(sprintf("DELETE FROM mail_routing_rule WHERE id IN (%s)", $ids_list));
	...
	public static function getSearchQueryComponents($columns, $params, $sortBy=null, $sortAsc=null) {
		$fields = SearchFields_MailRoutingRule::getFields();
		
		list(,$wheres) = parent::_parseSearchParams($params, $columns, 'SearchFields_MailRoutingRule', $sortBy);
		
		$select_sql = sprintf("SELECT ".
			"mail_routing_rule.id as %s, ".
			"mail_routing_rule.name as %s, ".
		...
		$sort_sql = self::_buildSortClause($sortBy, $sortAsc, $fields, $select_sql, 'SearchFields_MailRoutingRule');
		
		return [
			'primary_table' => 'mail_routing_rule',
			'select' => $select_sql,
			'join' => $join_sql,
			'where' => $where_sql,
			'sort' => $sort_sql,
...
class SearchFields_MailRoutingRule extends DevblocksSearchFields {
	const ID = 'm_id';
	const NAME = 'm_name';
	const IS_DISABLED = 'm_is_disabled';
	...
	static function getWhereSQL(DevblocksSearchCriteria $param) {
		switch($param->field) {
			case self::VIRTUAL_CONTEXT_LINK:
				return self::_getWhereSQLFromContextLinksField($param, Context_MailRoutingRule::ID, self::getPrimaryKey());
			
			case self::VIRTUAL_HAS_FIELDSET:
				return self::_getWhereSQLFromVirtualSearchSqlField($param, CerberusContexts::CONTEXT_CUSTOM_FIELDSET, sprintf('SELECT context_id FROM context_to_custom_fieldset WHERE context = %s AND custom_fieldset_id IN (%s)', Cerb_ORMHelper::qstr(Context_MailRoutingRule::ID), '%s'), self::getPrimaryKey());
	...
	static function getLabelsForKeyValues($key, $values) {
		switch($key) {
			case SearchFields_MailRoutingRule::ID:
				$models = DAO_MailRoutingRule::getIds($values);
...
class Model_MailRoutingRule extends DevblocksRecordModel {
	...
	function getKata(DevblocksDictionaryDelegate $dict) {
		//return DevblocksPlatform::services()->ui()->toolbar()->parse($this->routing_kata, $dict);
		// [TODO]
...
class View_MailRoutingRule extends C4_AbstractView implements IAbstractView_Subtotals, IAbstractView_QuickSearch {
	const DEFAULT_ID = 'mail_routing_rules';
	
	function __construct() {
		$this->id = self::DEFAULT_ID;
		$this->name = DevblocksPlatform::translateCapitalized('common.mail.routing.rules');
		$this->renderLimit = 25;
		$this->renderSortBy = SearchFields_MailRoutingRule::ID;
	...
	protected function _getData() {
		return DAO_MailRoutingRule::search(
			$this->view_columns,
			$this->getParams(),
			$this->renderLimit,
			$this->renderPage,
	...
	function getData() {
		...
		$this->_lazyLoadCustomFieldsIntoObjects($objects, 'SearchFields_MailRoutingRule');
		
		return $objects;
	}
	
	function getDataAsObjects($ids=null, &$total=null) {
		return $this->_getDataAsObjects('DAO_MailRoutingRule', $ids, $total);
	}
	
	function getDataSample($size) {
		return $this->_doGetDataSample('DAO_MailRoutingRule', $size);
	}
	...
	function getQuickSearchFields() {
		...
		$fields = [
			...
			'created' =>
				[
					...
					'options' => ['param_key' => SearchFields_MailRoutingRule::CREATED_AT],
				],
			'fieldset' =>
				[
			...
			'id' =>
				[
					...
					'options' => ['param_key' => SearchFields_MailRoutingRule::ID],
					'examples' => [
						['type' => 'chooser', 'context' => Context_MailRoutingRule::ID, 'q' => ''],
					]
				],
			'isDisabled' =>
				[
					'type' => DevblocksSearchCriteria::TYPE_BOOL,
					'options' => ['param_key' => SearchFields_MailRoutingRule::IS_DISABLED],
				],
			'name' =>
				[
					'type' => DevblocksSearchCriteria::TYPE_TEXT,
			...
			'priority' =>
				[
					'type' => DevblocksSearchCriteria::TYPE_NUMBER,
					'options' => ['param_key' => SearchFields_MailRoutingRule::PRIORITY],
				],
			'updated' =>
				[
					'type' => DevblocksSearchCriteria::TYPE_DATE,
		...
		$fields = self::_appendFieldsFromQuickSearchContext(Context_MailRoutingRule::ID, $fields, null);
	...
	function render() {
		...
		// Custom fields
		$custom_fields = DAO_CustomField::getByContext(Context_MailRoutingRule::ID);
		$tpl->assign('custom_fields', $custom_fields);
		
		$tpl->assign('view_template', 'devblocks:cerberusweb.core::records/types/mail_routing_rule/view.tpl');
	...
	function getFields() {
		return SearchFields_MailRoutingRule::getFields();
	}
	...
	function doSetCriteria($field, $oper, $value) {
		...
		switch($field) {
			case SearchFields_MailRoutingRule::NAME:
			...
			case SearchFields_MailRoutingRule::CREATED_AT:
			case SearchFields_MailRoutingRule::UPDATED_AT:
...
class Context_MailRoutingRule extends Extension_DevblocksContext implements IDevblocksContextProfile, IDevblocksContextPeek, IDevblocksContextWorkflow {
	...
	function getRandom() {
		return DAO_MailRoutingRule::random();
	}
	...
	function profileGetUrl($context_id) {
		...
		return $url_writer->writeNoProxy('c=profiles&type=mail_routing_rule&id='.$context_id, true);
	}
	...
	function profileGetFields($model=null) : array {
		...
		if(is_null($model))
			$model = new Model_MailRoutingRule();
	...
	function getMeta($context_id) {
		if(null == ($mail_routing_rule = DAO_MailRoutingRule::get($context_id)))
			return [];
		
		$url = $this->profileGetUrl($context_id);
		$friendly = DevblocksPlatform::strToPermalink($mail_routing_rule->name);
		
		if(!empty($friendly))
			$url .= '-' . $friendly;
		
		return [
			'id' => $mail_routing_rule->id,
			'name' => $mail_routing_rule->name,
			'permalink' => $url,
			'updated' => $mail_routing_rule->updated_at,
		];
	}
	...
	function getContext($mail_routing_rule, &$token_labels, &$token_values, $prefix=null) {
		if(is_null($prefix))
			$prefix = 'Mail Routing Rule:';
		
		$translate = DevblocksPlatform::getTranslationService();
		$fields = DAO_CustomField::getByContext(Context_MailRoutingRule::ID);
		
		// Polymorph
		if(is_numeric($mail_routing_rule)) {
			$mail_routing_rule = DAO_MailRoutingRule::get($mail_routing_rule);
		} elseif($mail_routing_rule instanceof Model_MailRoutingRule) {
			// It's what we want already.
			DevblocksPlatform::noop();
		} elseif(is_array($mail_routing_rule)) {
			$mail_routing_rule = Cerb_ORMHelper::recastArrayToModel($mail_routing_rule, 'Model_MailRoutingRule');
		} else {
			$mail_routing_rule = null;
		}
		...
		$token_values['_context'] = Context_MailRoutingRule::ID;
		$token_values['_type'] = 'mail_routing_rule';
		$token_values['_types'] = $token_types;
		
		if($mail_routing_rule) {
			$token_values['_loaded'] = true;
			$token_values['_label'] = $mail_routing_rule->name;
			$token_values['created_at'] = $mail_routing_rule->created_at;
			$token_values['id'] = $mail_routing_rule->id;
			$token_values['name'] = $mail_routing_rule->name;
			...
			// URL
			$url_writer = DevblocksPlatform::services()->url();
			$token_values['record_url'] = $url_writer->writeNoProxy(sprintf("c=profiles&type=mail_routing_rule&id=%d-%s",$mail_routing_rule->id, DevblocksPlatform::strToPermalink($mail_routing_rule->name)), true);
		}
	...
	function getKeyToDaoFieldMap() {
		return [
			'routing_kata' => DAO_MailRoutingRule::ROUTING_KATA,
			'id' => DAO_MailRoutingRule::ID,
			'is_disabled' => DAO_MailRoutingRule::IS_DISABLED,
			'links' => '_links',
	...
	function getChooserView($view_id=null) {
		...
		$view->name = 'Mail Routing Rule';
		/*
		$view->addParams([
			SearchFields_MailRoutingRule::UPDATED_AT => new DevblocksSearchCriteria(SearchFields_MailRoutingRule::UPDATED_AT,'=',0),
		], true);
		*/
		$view->renderSortBy = SearchFields_MailRoutingRule::UPDATED_AT;
	...
	function renderPeekPopup($context_id=0, $view_id='', $edit=false) {
		...
		if($context_id) {
			if(!($model = DAO_MailRoutingRule::get($context_id)))
		...
		if(empty($context_id) || $edit) {
			...
			$tpl->assign('view_id', $view_id);
			$tpl->display('devblocks:cerberusweb.core::records/types/mail_routing_rule/peek_edit.tpl');

...
class DAO_ConnectedService extends Cerb_ORMHelper {
	...
	static function search($columns, $params, $limit=10, $page=0, $sortBy=null, $sortAsc=null, $withCounts=true) {
		...
		while($row = mysqli_fetch_assoc($rs)) {
			$object_id = intval($row[SearchFields_ConnectedService::ID]);
		...
		if($withCounts) {
			...
			if(!(0 == $page && $total < $limit)) {
				$count_sql =
					"SELECT COUNT(connected_service.id) ".
					$join_sql.
					$where_sql;
...
class View_ConnectedService extends C4_AbstractView implements IAbstractView_Subtotals, IAbstractView_QuickSearch {
	...
	function getData() {
		$objects = DAO_ConnectedService::search(
			$this->view_columns,
			$this->getParams(),
			$this->renderLimit,
			$this->renderPage,
...
class Context_ConnectedService extends Extension_DevblocksContext implements IDevblocksContextProfile, IDevblocksContextPeek {
	...
	function renderPeekPopup($context_id=0, $view_id='', $edit=false) {
		...
		if(empty($context_id) || $edit) {
			...
			$tpl->display('devblocks:cerberusweb.core::internal/connected_service/peek.tpl');
		}
	}
};

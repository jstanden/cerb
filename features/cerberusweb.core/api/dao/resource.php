...
class SearchFields_Resource extends DevblocksSearchFields {
	...
	static function getPrimaryKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_Resource::ID);
	}
	
	static function getUpdatedKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_Resource::UPDATED_AT);
	}
		...
		switch($param->field) {
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, CerberusContexts::CONTEXT_RESOURCE, self::getPrimaryKey())))
...
class Storage_Resource extends Extension_DevblocksStorageSchema {
	const string ID = 'cerb.storage.schema.resources';
	...
	protected static function getArchiveCandidates(string $src_extension, int $src_profile_id, int $before, int $last_at, int $last_id, int $limit) : array {
		...
		// Resources still on the active profile, aged by `updated_at` (index `storage_profile_updated`)
		return $db->GetArrayReader(sprintf(
			"SELECT id, updated_at AS cursor_at FROM resource ".
			"WHERE storage_extension = %s AND storage_profile_id = %d ".
			"AND updated_at < %d AND (updated_at > %d OR (updated_at = %d AND id > %d)) ".

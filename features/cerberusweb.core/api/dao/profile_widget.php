...
class DAO_ProfileWidget extends Cerb_ORMHelper {
	...
	public static function getSearchQueryComponents($columns, $params, $sortBy=null, $sortAsc=null) {
		...
		array_walk_recursive(
			$params,
			array('DAO_ProfileWidget', '_translateVirtualParameters'),
			$args
		);
	...
	private static function _translateVirtualParameters($param, $key, &$args) {
		...
		$from_context = CerberusContexts::CONTEXT_PROFILE_WIDGET;
		$from_index = 'profile_widget.id';
	...
	static function search($columns, $params, $limit=10, $page=0, $sortBy=null, $sortAsc=null, $withCounts=true) {
		...
		while($row = mysqli_fetch_assoc($rs)) {
			$object_id = intval($row[SearchFields_ProfileWidget::ID]);
		...
		if($withCounts) {
			...
			if(!(0 == $page && $total < $limit)) {
				$count_sql =
					"SELECT COUNT(profile_widget.id) ".
					$join_sql.
					$where_sql;
...
class View_ProfileWidget extends C4_AbstractView implements IAbstractView_Subtotals, IAbstractView_QuickSearch {
	...
	function getData() {
		$objects = DAO_ProfileWidget::search(
			$this->view_columns,
			$this->getParams(),
			$this->renderLimit,
			$this->renderPage,
...
//				break;
	...
	function renderCriteria($field) {
		...
		switch($field) {
			...
			case SearchFields_ProfileWidget::VIRTUAL_HAS_FIELDSET:
				$this->_renderCriteriaHasFieldset($tpl, CerberusContexts::CONTEXT_PROFILE_WIDGET);
				break;
...
class Context_ProfileWidget extends Extension_DevblocksContext implements IDevblocksContextProfile, IDevblocksContextPeek {
	...
	function renderPeekPopup($context_id=0, $view_id='', $edit=false) {
		...
		if(empty($context_id) || $edit) {
			...
			$tpl->display('devblocks:cerberusweb.core::internal/profiles/widgets/peek.tpl');
		}
	}
};

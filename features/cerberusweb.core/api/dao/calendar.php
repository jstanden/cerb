...
class Context_Calendar extends Extension_DevblocksContext implements IDevblocksContextProfile, IDevblocksContextPeek { // IDevblocksContextImport
	...
	function importGetKeys() {
		...
		$keys = array(
			'name' => array(
				...
				'param' => SearchFields_Calendar::NAME,
				'required' => true,
			),
			'updated_at' => array(
				'label' => 'Updated Date',
				'type' => Model_CustomField::TYPE_DATE,
				'param' => SearchFields_Calendar::UPDATED_AT,
			),
		);
	
		$cfields = DAO_CustomField::getByContext(CerberusContexts::CONTEXT_CALENDAR);
	...
	function importSaveObject(array $fields, array $custom_fields, array $meta) {
		...
		if(!isset($meta['object_id']) || empty($meta['object_id'])) {
			...
			if(!isset($fields[DAO_Calendar::NAME])) {
				$fields[DAO_Calendar::NAME] = 'New ' . $this->manifest->name;
			}
	
			// Create
			$meta['object_id'] = DAO_Calendar::create($fields);
	
		} else {
			// Update
			DAO_Calendar::update($meta['object_id'], $fields);
		}

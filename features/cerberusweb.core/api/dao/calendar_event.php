...
class SearchFields_CalendarEvent extends DevblocksSearchFields {
	...
	static function getPrimaryKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_CalendarEvent::ID);
	}
		...
		switch($param->field) {
			...
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, CerberusContexts::CONTEXT_CALENDAR_EVENT, self::getPrimaryKey())))
	...
	static function _getFields() {
		...
		$columns = [
			...
			self::CALENDAR_ID => new DevblocksSearchField(self::CALENDAR_ID, 'calendar_event', 'calendar_id', $translate->_('common.calendar'), Model_CustomField::TYPE_NUMBER, true),
...
class View_CalendarEvent extends C4_AbstractView implements IAbstractView_Subtotals, IAbstractView_QuickSearch {
	...
	function getDataAsObjects($ids=null, &$total=null) {
		return $this->_getDataAsObjects('DAO_CalendarEvent', $ids, $total);
	}

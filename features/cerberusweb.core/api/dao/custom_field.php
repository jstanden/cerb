		...
		switch($param->field) {
			...
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, CerberusContexts::CONTEXT_CUSTOM_FIELD, self::getPrimaryKey())))
...
class Context_CustomField extends Extension_DevblocksContext implements IDevblocksContextPeek, IDevblocksContextProfile, IDevblocksContextWorkflow {
	...
	function renderPeekPopup($context_id=0, $view_id='', $edit=false) {
		...
		if(!$context_id || $edit) {
			...
			// Workflow-managed custom fields are version-controlled; warn that hand-edits get overwritten.
			$workflow_id = DAO_WorkflowResource::getWorkflowIdByRecord(Context_CustomField::ID, $model->id);

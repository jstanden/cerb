...
class DAO_Currency extends Cerb_ORMHelper {
	...
	static function search($columns, $params, $limit=10, $page=0, $sortBy=null, $sortAsc=null, $withCounts=true) {
		...
		while($row = mysqli_fetch_assoc($rs)) {
			$object_id = intval($row[SearchFields_Currency::ID]);
	...
	static function getWhereSQL(DevblocksSearchCriteria $param) {
		switch($param->field) {
			...
			case self::VIRTUAL_HAS_FIELDSET:
				return self::_getWhereSQLFromVirtualSearchSqlField($param, CerberusContexts::CONTEXT_CUSTOM_FIELDSET, sprintf('SELECT context_id FROM context_to_custom_fieldset WHERE context = %s AND custom_fieldset_id IN (%%s)', Cerb_ORMHelper::qstr(CerberusContexts::CONTEXT_CURRENCY)), self::getPrimaryKey());
				break;
...
class Context_Currency extends Extension_DevblocksContext implements IDevblocksContextProfile, IDevblocksContextPeek {
	...
	function getContext($currency, &$token_labels, &$token_values, $prefix=null) {
		...
		$token_values['_context'] = CerberusContexts::CONTEXT_CURRENCY;
	...
	function renderPeekPopup($context_id=0, $view_id='', $edit=false) {
		...
		if(empty($context_id) || $edit) {
			...
			$tpl->display('devblocks:cerberusweb.core::internal/currency/peek.tpl');
		}
	}
};

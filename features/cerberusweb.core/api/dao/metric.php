...
class DAO_Metric extends Cerb_ORMHelper {
	...
	static function autocomplete($term, $as='models') {
		...
		switch($as) {
			...
			default:
				return DAO_Metric::getIds(array_keys($objects));
		}
	}
...
class SearchFields_Metric extends DevblocksSearchFields {
	...
	static function getPrimaryKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_Metric::ID);
	}
	
	static function getUpdatedKey() : string {
		return sprintf('%s.%s', self::getTableName(), DAO_Metric::UPDATED_AT);
	}
		...
		switch($param->field) {
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, CerberusContexts::CONTEXT_METRIC, self::getPrimaryKey())))
...
class Context_Metric extends Extension_DevblocksContext implements IDevblocksContextProfile, IDevblocksContextPeek, IDevblocksContextAutocomplete, IDevblocksContextWorkflow {
	...
	public function autocomplete($term, $query = null) {
		...
		$results = DAO_Metric::autocomplete($term);

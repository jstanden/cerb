...
class DAO_ClassifierExample extends Cerb_ORMHelper {
	...
	public static function getSearchQueryComponents($columns, $params, $sortBy=null, $sortAsc=null) {
		...
		array_walk_recursive(
			$params,
			array('DAO_ClassifierExample', '_translateVirtualParameters'),
			$args
		);
	...
	private static function _translateVirtualParameters($param, $key, &$args) {
		...
		$from_context = CerberusContexts::CONTEXT_CLASSIFIER_EXAMPLE;
		$from_index = 'classifier_example.id';
	...
	static function search($columns, $params, $limit=10, $page=0, $sortBy=null, $sortAsc=null, $withCounts=true) {
		...
		while($row = mysqli_fetch_assoc($rs)) {
			$object_id = intval($row[SearchFields_ClassifierExample::ID]);
		...
		if($withCounts) {
			...
			if(!(0 == $page && $total < $limit)) {
				$count_sql =
					"SELECT COUNT(classifier_example.id) ".
					$join_sql.
					$where_sql;
...
class View_ClassifierExample extends C4_AbstractView implements IAbstractView_Subtotals, IAbstractView_QuickSearch {
	...
	function getData() {
		$objects = DAO_ClassifierExample::search(
			$this->view_columns,
			$this->getParams(),
			$this->renderLimit,
			$this->renderPage,
...
class Context_ClassifierExample extends Extension_DevblocksContext implements IDevblocksContextProfile, IDevblocksContextPeek { // IDevblocksContextImport
	...
	function getContext($classifier_example, &$token_labels, &$token_values, $prefix=null) {
		...
		$token_values['_context'] = CerberusContexts::CONTEXT_CLASSIFIER_EXAMPLE;

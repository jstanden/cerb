...
class DAO_ContextAvatar extends Cerb_ORMHelper {
	...
	static function create($fields) {
		...
		CerberusContexts::checkpointCreations(CerberusContexts::CONTEXT_CONTEXT_AVATAR, $id);
		...
		switch($param->field) {
			default:
				if(DevblocksPlatform::strStartsWith($param->field, 'cf_')) {
					...
					if(null !== ($virtual_where_sql = self::_getWhereSQLForCommonVirtual($param, CerberusContexts::CONTEXT_CONTEXT_AVATAR, self::getPrimaryKey())))
...
class Storage_ContextAvatar extends Extension_DevblocksStorageSchema {
	...
	protected static function getArchiveCandidates(string $src_extension, int $src_profile_id, int $before, int $last_at, int $last_id, int $limit) : array {
		...
		// Avatars still on the active profile, aged by `updated_at` (index `storage_profile_updated`)
		return $db->GetArrayReader(sprintf(
			"SELECT id, updated_at AS cursor_at FROM context_avatar ".

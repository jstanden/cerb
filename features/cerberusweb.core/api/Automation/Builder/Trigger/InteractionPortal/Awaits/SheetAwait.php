...
class SheetAwait extends AbstractAwait {
	function invoke(string $prompt_key, string $prompt_action, array $prompt_params, Model_AutomationContinuation $continuation) {
		switch($prompt_action) {
			case 'refresh':
				return $this->_promptAction_refresh($prompt_key, $prompt_params, $continuation);
		}
		return false;
	}
	...
	function render(Model_AutomationContinuation $continuation) {
		...
		$tpl->display('devblocks:cerberusweb.core::automations/triggers/interaction.portal/await/sheet.tpl');
	}
	
	private function _promptAction_refresh(string $prompt_key, array $prompt_params, Model_AutomationContinuation $continuation) {
		$tpl = DevblocksPlatform::services()->template();
		
		//var_dump($prompt_params);
		
		$page = $prompt_params['page'] ?? 0;
		$filter = $prompt_params['filter'] ?? null;
		...
		// [TODO] Convert 'scale' to 'grid' 
		if($layout_style == 'scale') {
			$tpl->display('devblocks:cerberusweb.core::automations/triggers/interaction.portal/await/sheet/render_scale.tpl');
		} else if(in_array($layout_style,  ['columns', 'grid'])) {
			$tpl->display('devblocks:cerberusweb.core::automations/triggers/interaction.portal/await/sheet/render_grid.tpl');
		} else if($layout_style == 'fieldsets') {
			$tpl->display('devblocks:cerberusweb.core::automations/triggers/interaction.portal/await/sheet/render_fieldsets.tpl');
		} else {
			$tpl->display('devblocks:cerberusweb.core::automations/triggers/interaction.portal/await/sheet/render.tpl');
		}

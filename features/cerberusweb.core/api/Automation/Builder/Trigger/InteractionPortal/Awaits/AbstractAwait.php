...
abstract class AbstractAwait {
	...
	abstract function validate(_DevblocksValidationService $validation, Model_AutomationContinuation $continuation);
	abstract function formatValue();
	abstract function render(Model_AutomationContinuation $continuation);
	abstract function invoke(string $prompt_key, string $prompt_action, array $prompt_params, Model_AutomationContinuation $continuation);

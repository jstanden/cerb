...
class CardWidget_FormInteraction extends Extension_CardWidget {
	...
	private function _cardWidgetConfig_previewInteractions(Model_CardWidget $model) {
		...
		$interactions = $this->getInteractions($model, $dict);
		
		$tpl->assign('interactions', $interactions);
		$tpl->display('devblocks:cerberusweb.core::internal/cards/widgets/form_interaction/config_preview.tpl');
	}
	...
	function render(Model_CardWidget $widget, $context, $context_id) {
		...
		$tpl->display('devblocks:cerberusweb.core::internal/cards/widgets/form_interaction/render.tpl');
	}
	
	private function _getStateKey(Model_CardWidget $widget, DevblocksDictionaryDelegate $dict) {
		$worker = CerberusApplication::getActiveWorker();
		
		$state_key = 'form:card:' . sha1(sprintf("%d:%d:%s:%d",
			$widget->id,
	...
	function getInteractions(Model_CardWidget $widget, DevblocksDictionaryDelegate $dict) {
		...
		if(false == ($interactions = DevblocksPlatform::services()->kata()->parse($interactions_kata, $error))) {
			return false;
		}
		
		$interactions = DevblocksPlatform::services()->kata()->formatTree($interactions, $dict);
		...
		foreach($interactions as $interaction_key => $interaction) {
			$interaction['key'] = DevblocksPlatform::services()->string()->strBefore($interaction_key, '/');
			$results[$interaction_key] = $interaction;
		}
	...
	function renderForm(Model_CardWidget $widget, DevblocksDictionaryDelegate $dict, $is_submit=false) {
		...
		if(!$state_id) {
			try {
				...
				if(!array_key_exists('key', $interaction))
					throw new Exception();
				
				if(false == ($interaction_behavior = DAO_TriggerEvent::getByUri($interaction['key'])))
					throw new Exception();
				
				$interaction_behavior_vars = @$interaction['params'] ?: [];
	...
	private function _startFormSession(Model_CardWidget $widget, DevblocksDictionaryDelegate $dict, Model_TriggerEvent $interaction_behavior, array $interaction_behavior_vars=[]) {
		...
		foreach($actions as $params) {
			switch(@$params['_action']) {
				...
				default:
					...
					$tpl->display('devblocks:cerberusweb.core::internal/cards/widgets/form_interaction/continue.tpl');
					break;
			}
		}

{$page_context = CerberusContexts::CONTEXT_CUSTOM_FIELDSET}
{$page_context_id = $custom_fieldset->id}
{$is_writeable = Context_CustomFieldset::isWriteableByActor($custom_fieldset, $active_worker)}

<h1>{$custom_fieldset->name}</h1>
...
<div class="cerb-profile-toolbar">
	<form class="toolbar" action="{devblocks_url}{/devblocks_url}" onsubmit="return false;" style="margin-bottom:5px;">
		...
		<button type="button" id="btnDisplayCustomFieldsetEdit" title="{'common.edit'|devblocks_translate|capitalize} (E)" class="cerb-peek-trigger" data-context="{$page_context}" data-context-id="{$page_context_id}" data-edit="true"><span class="glyphicons glyphicons-cogwheel"></span></button>
		{/if}
	</form>
...
<fieldset class="properties">
	<legend>{'Custom Fieldset'|devblocks_translate|capitalize}</legend>
...
$(function() {
	...
	tabOptions.active = Devblocks.getjQueryUiTabSelected('custom_fieldsetTabs');

	var tabs = $("#custom_fieldsetTabs").tabs(tabOptions);

	$('#btnProfileCard').cerbPeekTrigger();
	
	// Edit
	{if $is_writeable && $active_worker->hasPriv("contexts.{$page_context}.update")}
	$('#btnDisplayCustomFieldsetEdit')
		.cerbPeekTrigger()
	...
	$(document).keypress(function(event) {
		...
		switch(event.which) {
			...
			case 58:  // (0) tab cycle
				try {
					idx = event.which-49;
					$tabs = $("#custom_fieldsetTabs").tabs();
					$tabs.tabs('option', 'active', idx);
				} catch(ex) { }
				break;
			case 101:  // (E) edit
				try {
					$('#btnDisplayCustomFieldsetEdit').click();
				} catch(ex) { }
				break;
			case 109:  // (M) macros
				try {

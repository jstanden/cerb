...
<fieldset class="properties" style="margin-top:5px;">
	<legend>{'common.behavior'|devblocks_translate|capitalize}</legend>
...
$(function() {
	...
	tabOptions.active = Devblocks.getjQueryUiTabSelected('trigger_eventTabs');

	var tabs = $("#trigger_eventTabs").tabs(tabOptions);

	$('#btnProfileCard').cerbPeekTrigger();
	
	// Edit
	
	$('#btnDisplayTriggerEventEdit')
		.cerbPeekTrigger()
	...
	$(document).keypress(function(event) {
		...
		switch(event.which) {
			...
			case 58:  // (0) tab cycle
				try {
					idx = event.which-49;
					$tabs = $("#trigger_eventTabs").tabs();
					$tabs.tabs('option', 'active', idx);
				} catch(ex) { }
				break;
			case 101:  // (E) edit
				try {
					$('#btnDisplayTriggerEventEdit').click();
				} catch(ex) { }
				break;
			case 109:  // (M) macros
				try {

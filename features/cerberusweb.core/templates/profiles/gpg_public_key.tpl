{$page_context = CerberusContexts::CONTEXT_GPG_PUBLIC_KEY}
{$page_context_id = $gpg_public_key->id}
{$is_writeable = Context_GpgPublicKey::isWriteableByActor($gpg_public_key, $active_worker)}

<h1>{$gpg_public_key->name}</h1>
...
<div class="cerb-profile-toolbar">
	<form class="toolbar" action="{devblocks_url}{/devblocks_url}" onsubmit="return false;" style="margin-bottom:5px;">
		...
		<button type="button" id="btnDisplayGpgPublicKeyEdit" title="{'common.edit'|devblocks_translate|capitalize} (E)" class="cerb-peek-trigger" data-context="{$page_context}" data-context-id="{$page_context_id}" data-edit="true"><span class="glyphicons glyphicons-cogwheel"></span></button>
		{/if}
	</form>
...
<fieldset class="properties">
	<legend>{'Gpg Public Key'|devblocks_translate|capitalize}</legend>
...
$(function() {
	...
	tabOptions.active = Devblocks.getjQueryUiTabSelected('gpg_public_keyTabs');

	var tabs = $("#gpg_public_keyTabs").tabs(tabOptions);

	$('#btnProfileCard').cerbPeekTrigger();
	
	// Edit
	{if $is_writeable && $active_worker->hasPriv("contexts.{$page_context}.update")}
	$('#btnDisplayGpgPublicKeyEdit')
		.cerbPeekTrigger()
	...
	$(document).keypress(function(event) {
		...
		switch(event.which) {
			...
			case 58:  // (0) tab cycle
				try {
					idx = event.which-49;
					$tabs = $("#gpg_public_keyTabs").tabs();
					$tabs.tabs('option', 'active', idx);
				} catch(ex) { }
				break;
			case 101:  // (E) edit
				try {
					$('#btnDisplayGpgPublicKeyEdit').click();
				} catch(ex) { }
				break;
			case 109:  // (M) macros
				try {

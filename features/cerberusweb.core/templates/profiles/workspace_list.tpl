{$page_context = CerberusContexts::CONTEXT_WORKSPACE_WORKLIST}
{$page_context_id = $workspace_list->id}
{$is_writeable = Context_WorkspaceList::isWriteableByActor($workspace_list, $active_worker)}

<h1>{$workspace_list->name}</h1>
...
<div class="cerb-profile-toolbar">
	<form class="toolbar" action="{devblocks_url}{/devblocks_url}" onsubmit="return false;" style="margin-bottom:5px;">
		...
		<button type="button" id="btnDisplayWorkspaceListEdit" title="{'common.edit'|devblocks_translate|capitalize} (E)" class="cerb-peek-trigger" data-context="{$page_context}" data-context-id="{$page_context_id}" data-edit="true"><span class="glyphicons glyphicons-cogwheel"></span></button>
		{/if}
	</form>
...
<fieldset class="properties">
	<legend>{'Workspace List'|devblocks_translate|capitalize}</legend>
...
$(function() {
	...
	tabOptions.active = Devblocks.getjQueryUiTabSelected('workspace_listTabs');

	var tabs = $("#workspace_listTabs").tabs(tabOptions);

	$('#btnProfileCard').cerbPeekTrigger();
	
	// Edit
	{if $is_writeable && $active_worker->hasPriv("contexts.{$page_context}.update")}
	$('#btnDisplayWorkspaceListEdit')
		.cerbPeekTrigger()
	...
	$(document).keypress(function(event) {
		...
		switch(event.which) {
			...
			case 58:  // (0) tab cycle
				try {
					idx = event.which-49;
					$tabs = $("#workspace_listTabs").tabs();
					$tabs.tabs('option', 'active', idx);
				} catch(ex) { }
				break;
			case 101:  // (E) edit
				try {
					$('#btnDisplayWorkspaceListEdit').click();
				} catch(ex) { }
				break;
			case 109:  // (M) macros
				try {

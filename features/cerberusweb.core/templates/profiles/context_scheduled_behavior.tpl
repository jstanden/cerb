{$page_context = CerberusContexts::CONTEXT_BEHAVIOR_SCHEDULED}
{$page_context_id = $context_scheduled_behavior->id}
{$is_writeable = Context_ContextScheduledBehavior::isWriteableByActor($context_scheduled_behavior, $active_worker)}
...
<div class="cerb-profile-toolbar">
	<form class="toolbar" action="{devblocks_url}{/devblocks_url}" onsubmit="return false;" style="margin-bottom:5px;">
		...
		<button type="button" id="btnDisplayContextScheduledBehaviorEdit" title="{'common.edit'|devblocks_translate|capitalize} (E)" class="cerb-peek-trigger" data-context="{$page_context}" data-context-id="{$page_context_id}" data-edit="true"><span class="glyphicons glyphicons-cogwheel"></span></button>
		{/if}
	</form>
...
<fieldset class="properties">
	<legend>{'common.behavior.scheduled'|devblocks_translate|capitalize}</legend>
...
$(function() {
	...
	tabOptions.active = Devblocks.getjQueryUiTabSelected('tabsScheduledBehavior');

	var tabs = $("#tabsScheduledBehavior").tabs(tabOptions);

	$('#btnProfileCard').cerbPeekTrigger();
	
	// Edit
	{if $is_writeable && $active_worker->hasPriv("contexts.{$page_context}.update")}
	$('#btnDisplayContextScheduledBehaviorEdit')
		.cerbPeekTrigger()
	...
	$(document).keypress(function(event) {
		...
		switch(event.which) {
			...
			case 58:  // (0) tab cycle
				try {
					idx = event.which-49;
					$tabs = $("#tabsScheduledBehavior").tabs();
					$tabs.tabs('option', 'active', idx);
				} catch(ex) { }
				break;
			case 101:  // (E) edit
				try {
					$('#btnDisplayContextScheduledBehaviorEdit').click();
				} catch(ex) { }
				break;
			case 109:  // (M) macros
				try {

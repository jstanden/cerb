{$page_context = CerberusContexts::CONTEXT_CLASSIFIER_ENTITY}
{$page_context_id = $classifier_entity->id}
{$is_writeable = Context_ClassifierEntity::isWriteableByActor($classifier_entity, $active_worker)}

<div style="float:left">
	<h1>{$classifier_entity->name}</h1>
</div>
...
<fieldset class="properties">
	<legend>{'Classifier Entity'|devblocks_translate|capitalize}</legend>
...
$(function() {
	...
	tabOptions.active = Devblocks.getjQueryUiTabSelected('classifier_entityTabs');

	var tabs = $("#classifier_entityTabs").tabs(tabOptions);

	// Edit
	{if $is_writeable}
	$('#btnDisplayClassifierEntityEdit')
		.cerbPeekTrigger()
	...
	$(document).keypress(function(event) {
		...
		switch(event.which) {
			...
			case 58:  // (0) tab cycle
				try {
					idx = event.which-49;
					$tabs = $("#classifier_entityTabs").tabs();
					$tabs.tabs('option', 'active', idx);
				} catch(ex) { }
				break;
			case 101:  // (E) edit
				try {
					$('#btnDisplayClassifierEntityEdit').click();
				} catch(ex) { }
				break;
			case 109:  // (M) macros
				try {

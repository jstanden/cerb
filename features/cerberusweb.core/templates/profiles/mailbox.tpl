...
<fieldset class="properties">
	<legend>{'common.mailbox'|devblocks_translate|capitalize}</legend>
...
$(function() {
	...
	tabOptions.active = Devblocks.getjQueryUiTabSelected('mailboxTabs');

	var tabs = $("#mailboxTabs").tabs(tabOptions);
	
	$('#btnDisplayMailboxEdit').bind('click', function() {
		$popup = genericAjaxPopup('peek','c=internal&a=showPeekPopup&context={$page_context}&context_id={$page_context_id}',null,false,'50%');
		$popup.one('mailbox_save', function(event) {
...
$(document).keypress(function(event) {
	...
	switch(event.which) {
		...
		case 58:  // (0) tab cycle
			try {
				idx = event.which-49;
				$tabs = $("#mailboxTabs").tabs();
				$tabs.tabs('option', 'active', idx);
			} catch(ex) { }
			break;
		case 101:  // (E) edit
			try {
				$('#btnDisplayMailboxEdit').click();
			} catch(ex) { }
			break;
		case 109:  // (M) macros
			try {

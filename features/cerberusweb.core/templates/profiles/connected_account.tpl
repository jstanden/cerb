{$page_context = CerberusContexts::CONTEXT_CONNECTED_ACCOUNT}
{$page_context_id = $connected_account->id}
{$is_readable = Context_ConnectedAccount::isReadableByActor($connected_account, $active_worker)}
{$is_writeable = Context_ConnectedAccount::isWriteableByActor($connected_account, $active_worker)}

<div style="float:left">
	<h1>{$connected_account->name}</h1>
</div>
...
<fieldset class="properties">
	<legend>{'common.connected_account'|devblocks_translate|capitalize}</legend>
...
$(function() {
	...
	tabOptions.active = Devblocks.getjQueryUiTabSelected('connected_accountTabs');
...
$(document).keypress(function(event) {
	...
	switch(event.which) {
		...
		case 58:  // (0) tab cycle
			try {
				idx = event.which-49;
				$tabs = $("#connected_accountTabs").tabs();
				$tabs.tabs('option', 'active', idx);
			} catch(ex) { }
			break;
		case 101:  // (E) edit
			try {
				$('#btnDisplayConnectedAccountEdit').click();
			} catch(ex) { }
			break;
		case 109:  // (M) macros
			try {

{$page_context = CerberusContexts::CONTEXT_MAIL_TRANSPORT}
{$page_context_id = $mail_transport->id}
{$is_writeable = Context_MailTransport::isWriteableByActor($mail_transport, $active_worker)}
...
<div class="cerb-profile-toolbar">
	<form class="toolbar" action="{devblocks_url}{/devblocks_url}" onsubmit="return false;" style="margin-bottom:5px;">
		...
		<button type="button" id="btnDisplayMailTransportEdit" title="{'common.edit'|devblocks_translate|capitalize} (E)" class="cerb-peek-trigger" data-context="{$page_context}" data-context-id="{$page_context_id}" data-edit="true"><span class="glyphicons glyphicons-cogwheel"></span></button>
		{/if}
...
<fieldset class="properties">
	<legend>{'common.email_transport'|devblocks_translate|capitalize}</legend>
...
$(function() {
	...
	tabOptions.active = Devblocks.getjQueryUiTabSelected('mail_transportTabs');

	var tabs = $("#mail_transportTabs").tabs(tabOptions);

	$('#btnProfileCard').cerbPeekTrigger();
	
	// Edit
	{if $is_writeable && $active_worker->hasPriv("contexts.{$page_context}.update")}
	$('#btnDisplayMailTransportEdit')
		.cerbPeekTrigger()

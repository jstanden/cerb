{$page_context = CerberusContexts::CONTEXT_ROLE}
{$page_context_id = $worker_role->id}
{$is_writeable = Context_WorkerRole::isWriteableByActor($worker_role, $active_worker)}

<div style="float:left">
	<h1>{$worker_role->name}</h1>
</div>
...
<div class="cerb-profile-toolbar">
	<form class="toolbar" action="{devblocks_url}{/devblocks_url}" onsubmit="return false;" style="margin-bottom:5px;">
		...
		{*
		{if $is_writeable}
		{devblocks_url assign=return_url full=true}c=profiles&type=worker_role&id={$page_context_id}-{$worker_role->name|devblocks_permalink}{/devblocks_url}
		{include file="devblocks:cerberusweb.core::internal/macros/display/button.tpl" context=$page_context context_id=$page_context_id macro_event="event.macro.role" return_url=$return_url}
		{/if}
		*}
		
		<!-- Edit -->
		{if $is_writeable}
		<button type="button" id="btnDisplayWorkerRoleEdit" title="{'common.edit'|devblocks_translate|capitalize} (E)" class="cerb-peek-trigger" data-context="{$page_context}" data-context-id="{$page_context_id}" data-edit="true"><span class="glyphicons glyphicons-cogwheel"></span></button>
		{/if}
	</form>
...
<fieldset class="properties">
	<legend>{'common.role'|devblocks_translate|capitalize}</legend>
...
$(function() {
	...
	tabOptions.active = Devblocks.getjQueryUiTabSelected('workerRoleTabs');

	var tabs = $("#workerRoleTabs").tabs(tabOptions);

	// Edit
	{if $is_writeable}
	$('#btnDisplayWorkerRoleEdit')
		.cerbPeekTrigger()
	...
	$(document).keypress(function(event) {
		...
		switch(event.which) {
			...
			case 58:  // (0) tab cycle
				try {
					idx = event.which-49;
					$tabs = $("#workerRoleTabs").tabs();
					$tabs.tabs('option', 'active', idx);
				} catch(ex) { }
				break;
			case 101:  // (E) edit
				try {
					$('#btnDisplayWorkerRoleEdit').click();
				} catch(ex) { }
				break;
			case 109:  // (M) macros
				try {

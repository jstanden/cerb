...
<div id="divContextLinkBuffer">
	{foreach from=$links item=link key=link_id}
		<div>
			<a href="javascript:;" onclick="genericAjaxGet('','c=internal&a=contextDeleteLink&context={$from_context|escape}&context_id={$from_context_id|escape}&dst_context={$to_context|escape}&dst_context_id={$link_id|escape}');$(this).parent().remove();">X</a> {$link->getName()|escape}
...
<script language="JavaScript1.2" type="text/javascript">
	...
	genericPanel.one('dialogopen',function(event,ui) {
		genericPanel.dialog('option','title','Link Workers');

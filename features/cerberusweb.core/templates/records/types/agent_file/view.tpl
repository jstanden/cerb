{$view_context = 'cerb.contexts.agent.file'}
	...
	<tbody style="cursor:pointer;">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td>
				...
				<a href="{devblocks_url}c=profiles&type=agent_file&id={$result.a_id}-{$result.a_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.a_name}</a>
			...
			{elseif $column == "a_filesystem_id"}
				<td>
					{$fs_id = $result.a_filesystem_id}
					{if $fs_id && isset($filesystems.$fs_id)}
						{$fs = $filesystems.$fs_id}
						<a class="cerb-peek-trigger no-underline" data-context="cerb.contexts.agent.filesystem" data-context-id="{$fs->id}">{$fs->name}</a>
					{/if}
				</td>
			{elseif $column == "a_size"}
				<td>{if !empty($result.a_size)}{$result.a_size|devblocks_prettybytes}{/if}</td>
			{else}
...
<div style="padding-top:5px;">
	...
	<div style="float:left;" id="{$view->id}_actions">
		...
		{if $active_worker->is_superuser}<button data-cerb-worklist-action-bulk="agent_file" type="button" class="action-always-show action-bulkupdate"><span class="cerb-icons cerb-icon-folder"></span> {'common.bulk_update'|devblocks_translate|lower}</button>{/if}
	</div>
</div>
{/if}

{$bulk_context = 'cerb.contexts.agent.file'}
	...
	<table cellspacing="0" cellpadding="2" width="100%">
		<tr>
			<td width="0%" nowrap="nowrap" align="left" valign="middle">
				<label>
					<input type="checkbox" name="actions[]" value="filesystem_id">
					{'dao.agent_file.filesystem_id'|devblocks_translate|capitalize}:
				</label>
			</td>
			<td width="100%">
				<div style="display:none;">
					<div class="cerb-ui-record-chooser" data-cerb-chooser="params_filesystem_id"></div>
					<div class="cerb-ui-form--help">Files whose path is already taken on the destination filesystem are skipped.</div>
				</div>
			</td>
		</tr>
...
{if $active_worker->hasPriv("contexts.{$bulk_context}.delete")}
	<fieldset class="peek" data-cerb-section-name="delete">
		...
		<div style="display:none;margin-left:10px;">
			The selected agent files will be permanently deleted.
		</div>
	</fieldset>
{/if}
...
$(function() {
	...
	$popup.one('popup_open', function() {
		...
		if(window.CerbUI && CerbUI.RecordChooser) {
			new CerbUI.RecordChooser($popup.find('[data-cerb-chooser="params_filesystem_id"]')[0], { context: 'agent_filesystem', name: 'params[filesystem_id]', emptyIcon: 'folder' });

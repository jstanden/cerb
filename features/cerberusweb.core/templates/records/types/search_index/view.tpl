{$view_context = 'cerb.contexts.search.index'}
        ...
        {foreach from=$data item=result key=idx name=results}
            ...
            <tr class="{$tableRowClass}">
                <td data-column="*_watchers" align="center" nowrap="nowrap" style="padding:5px;">
                    {include file="devblocks:cerberusweb.core::internal/watchers/context_follow_button.tpl" context=$view_context context_id=$result.r_id}
                </td>
                {foreach from=$view->view_columns item=column name=columns}
                    ...
                    {elseif $column == "r_name"}
                        <td>
                            ...
                            <a href="{devblocks_url}c=profiles&type=search_index&id={$result.r_id}-{$result.r_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.r_name}</a>
                            <button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.r_id}"><span class="cerb-icons cerb-icon-new-window"></span></button>
                        </td>
                    ...
                    {elseif $column == "*_sparkline"}
                        <td data-column="{$column}" style="width:160px;">
                            <div class="cerb-ui-sparkchart" data-cerb-spark="{$result.r_id}" style="min-width:140px;"></div>
                        </td>
                    {else}
...
<script nonce="{DevblocksPlatform::getRequestNonce()}" type="text/javascript">
    $(function() {
        ...
        {include file="devblocks:cerberusweb.core::internal/views/sparkline_loader.tpl" spark_module='search_index' spark_action='viewSparklinesJson' spark_view_id=$view->id spark_tooltip_labels=true}
        {/if}

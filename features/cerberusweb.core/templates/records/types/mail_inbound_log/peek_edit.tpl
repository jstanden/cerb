{$peek_context = 'cerb.contexts.mail.inbound.log'}
...
<form action="{devblocks_url}{/devblocks_url}" method="post" id="{$form_id}">
	...
	{if !empty($model->id)}
		{include file="devblocks:cerberusweb.core::internal/peek/delete_confirm.tpl" noun="mail inbound log entry"}
	{/if}
...
<script nonce="{DevblocksPlatform::getRequestNonce()}" type="text/javascript">
	$(function() {
		...
		$popup.one('popup_open', function() {
			$popup.dialog('option','title',"{'Mail Inbound Log'|devblocks_translate|capitalize|escape:'javascript' nofilter}");

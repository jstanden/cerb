{$view_context = 'cerb.contexts.mail.delivery.log'}
...
<form id="viewForm{$view->id}" name="viewForm{$view->id}" action="{devblocks_url}{/devblocks_url}" method="post">
    ...
    <table cellpadding="1" cellspacing="0" border="0" width="100%" class="worklistBody">
        {$status_labels = [0 => 'sent', 1 => 'delivered', 2 => 'error']}
        ...
        {foreach from=$data item=result key=idx name=results}
            ...
            {capture name="title_column"}
                ...
                <a href="{devblocks_url}c=profiles&type=mail_delivery_log&id={$result.m_id}-{$result.m_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.m_subject}</a>
            ...
            <tr class="{$tableRowClass}">
                {foreach from=$view->view_columns item=column name=columns}
                    ...
                    {elseif $column == "m_mail_transport_id"}
                        <td data-column="{$column}">
                            {if is_array($mail_transports) && array_key_exists($result.$column, $mail_transports)}
                                <a class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_MAIL_TRANSPORT}" data-context-id="{$result.$column}">{$mail_transports[$result.$column]->name}</a>
                            {elseif $result.$column}
                                {$result.$column}
                            {/if}
                        </td>
                    {elseif $column == "m_status_id"}
                        <td data-column="{$column}">
                            {if array_key_exists($result.$column, $status_labels)}
                            {$status_id = $result.$column}
                            <span class="tag" style="color:white;{if 2 == $result.$column}background-color:rgb(185,50,40);{else}background-color:rgb(100,140,25);{/if}">
                                {if 2 == $result.$column}failed{else}{'mail.sent'|devblocks_translate|lower}{/if}
                            </span>
                            {/if}
                        </td>

{$view_context = CerberusContexts::CONTEXT_AUTOMATION_EVENT_LISTENER}
        ...
        {foreach from=$data item=result key=idx name=results}
			...
			<tr class="{$tableRowClass}">
                ...
                {foreach from=$view->view_columns item=column name=columns}
					...
					{elseif in_array($column, ['a_is_disabled'])}
                    ...
                    {elseif $column == "a_name"}
						<td>
							...
							<a href="{devblocks_url}c=profiles&type=automation_event_listener&id={$result.a_id}-{$result.a_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.a_name}</a>

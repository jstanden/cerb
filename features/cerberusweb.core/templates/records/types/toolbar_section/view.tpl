{$view_context = CerberusContexts::CONTEXT_TOOLBAR_SECTION}
...
{$toolbar_name_params = $view->findParam('t_toolbar_name', $view->getParams())}
{foreach from=$toolbar_name_params item=p}
    {if $p->value}{$create_edit_defaults = "toolbar:{$p->value}"}{/if}
{/foreach}
...
<table cellpadding="0" cellspacing="0" border="0" class="worklist" width="100%" {if array_key_exists('header_color', $view->options) && $view->options.header_color}style="background-color:{$view->options.header_color};"{/if}>
    <tr>
        ...
        <td nowrap="nowrap" align="right" class="title-toolbar">
            {if $active_worker->hasPriv("contexts.{$view_context}.create")}<a title="{'common.add'|devblocks_translate|capitalize}" class="minimal peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="0" {if $create_edit_defaults}data-edit="{$create_edit_defaults}"{/if}><span class="cerb-icons cerb-icon-circle-plus"></span></a>{/if}
                ...
                {foreach from=$view->view_columns item=column name=columns}
                    ...
                    {elseif $column == "t_name"}
                        <td>
                            ...
                            <a href="{devblocks_url}c=profiles&type=toolbar_section&id={$result.t_id}-{$result.t_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.t_name}</a>
                    ...
                    {elseif in_array($column, ["t_created_at", "t_updated_at"])}
                        <td>
                            {if !empty($result.$column)}
                                <abbr title="{$result.$column|devblocks_date}">{$result.$column|devblocks_prettytime}</abbr>
                            {/if}
                        </td>
                    {elseif in_array($column, ['t_workflow_id'])}

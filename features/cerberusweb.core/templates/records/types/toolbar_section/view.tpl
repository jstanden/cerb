...
<table cellpadding="0" cellspacing="0" border="0" class="worklist" width="100%" {if array_key_exists('header_color', $view->options) && $view->options.header_color}style="background-color:{$view->options.header_color};"{/if}>
    <tr>
        ...
        <td nowrap="nowrap" align="right" class="title-toolbar">
            {if $active_worker->hasPriv("contexts.{$view_context}.create")}<a title="{'common.add'|devblocks_translate|capitalize}" class="minimal peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="0" {if $create_edit_defaults}data-edit="{$create_edit_defaults}"{/if}><span class="glyphicons glyphicons-circle-plus"></span></a>{/if}
                ...
                {foreach from=$view->view_columns item=column name=columns}
                    ...
                    {elseif $column == "t_name"}
                        <td>
                            ...
                            <button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.t_id}"><span class="glyphicons glyphicons-new-window-alt"></span></button>
                        </td>

{$view_context = CerberusContexts::CONTEXT_QUEUE}
        ...
        {foreach from=$data item=result key=idx name=results}
            ...
            <tr class="{$tableRowClass}">
                <td data-column="*_watchers" align="center" nowrap="nowrap" style="padding:5px;">
                    {include file="devblocks:cerberusweb.core::internal/watchers/context_follow_button.tpl" context=$view_context context_id=$result.q_id}
                </td>
                {foreach from=$view->view_columns item=column name=columns}
                    {if DevblocksPlatform::strStartsWith($column, "cf_")}
                        {include file="devblocks:cerberusweb.core::internal/custom_fields/view/cell_renderer.tpl"}
                    {elseif $column == "q_name"}
                        <td>
                            <input type="checkbox" name="row_id[]" value="{$result.q_id}" style="display:none;">
                            <a href="{devblocks_url}c=profiles&type=queue&id={$result.q_id}-{$result.q_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.q_name}</a>
                            <button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.q_id}"><span class="cerb-icons cerb-icon-new-window"></span></button>
                        </td>
                    {elseif in_array($column, ["q_created_at", "q_updated_at"])}
                        <td>
                    ...
                    {elseif $column == "*_sparkline"}
                        <td data-column="{$column}" style="width:160px;">
                            <div class="cerb-ui-sparkchart" data-cerb-spark="{$result.q_id}" style="min-width:140px;"></div>
                        </td>
                    {else}
...
<script nonce="{DevblocksPlatform::getRequestNonce()}" type="text/javascript">
    $(function() {
        ...
        {include file="devblocks:cerberusweb.core::internal/views/sparkline_loader.tpl" spark_module='queue' spark_action='viewSparklinesJson' spark_view_id=$view->id}
        {/if}

...
<table cellpadding="0" cellspacing="0" border="0" class="worklist" width="100%" {if array_key_exists('header_color', $view->options) && $view->options.header_color}style="background-color:{$view->options.header_color};"{/if}>
    <tr>
        ...
        <td nowrap="nowrap" align="right" class="title-toolbar">
            ...
            {if $active_worker->hasPriv("contexts.{$view_context}.import")}<a title="{$translate->_('common.import')|capitalize}" onclick="genericAjaxPopup('import','c=internal&a=invoke&module=worklists&action=renderImportPopup&context={$view_context}&view_id={$view->id}',null,false,'50%');"><span class="glyphicons glyphicons-file-import"></span></a>{/if}
                ...
                {foreach from=$view->view_columns item=column name=columns}
                    ...
                    {elseif $column == "q_name"}
                        <td>
                            ...
                            <button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.q_id}"><span class="glyphicons glyphicons-new-window-alt"></span></button>
                        </td>

{$view_context = CerberusContexts::CONTEXT_WORKFLOW}
...
<table cellpadding="0" cellspacing="0" border="0" class="worklist" width="100%" {if array_key_exists('header_color', $view->options) && $view->options.header_color}style="background-color:{$view->options.header_color};"{/if}>
    <tr>
        ...
        <td nowrap="nowrap" align="right" class="title-toolbar">
            {if $active_worker->hasPriv("contexts.{$view_context}.create")}<a title="{'common.add'|devblocks_translate|capitalize}" class="minimal peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="0" data-width="90%"><span class="cerb-icons cerb-icon-circle-plus"></span></a>{/if}
                ...
                {foreach from=$view->view_columns item=column name=columns}
                    ...
                    {elseif $column == "a_name"}
                        <td>
                            ...
                            <a href="{devblocks_url}c=profiles&type=workflow&id={$result.a_id}-{$result.a_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.a_name}</a>

{$view_context = CerberusContexts::CONTEXT_QUEUE_JOB}
	...
	<tbody style="cursor:pointer;">
		<tr class="{$tableRowClass}">
			<td data-column="*_watchers" align="center" nowrap="nowrap" style="padding:5px;">
				{include file="devblocks:cerberusweb.core::internal/watchers/context_follow_button.tpl" context=$view_context context_id=$result.qj_id}
			</td>
		{foreach from=$view->view_columns item=column name=columns}
			{if DevblocksPlatform::strStartsWith($column, "cf_")}
				{include file="devblocks:cerberusweb.core::internal/custom_fields/view/cell_renderer.tpl"}
			{elseif $column == "qj_name"}
			<td>
				<input type="checkbox" name="row_id[]" value="{$result.qj_id}" style="display:none;">
				<a href="{devblocks_url}c=profiles&type=queue_job&id={$result.qj_id}-{$result.qj_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.qj_name}</a>
				<button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.qj_id}"><span class="cerb-icons cerb-icon-new-window"></span></button>
			</td>
			{elseif $column == "qj_queue_id"}
			<td>
				{$_queue = $queues[$result.qj_queue_id]}
				{if $_queue}
					<a class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_QUEUE}" data-context-id="{$_queue->id}">{$_queue->name}</a>
				{/if}
			</td>
			{elseif $column == "qj_worker_id"}
			<td>
				{$_worker = $workers[$result.qj_worker_id]}
				{if $_worker instanceof Model_Worker}
					<img src="{devblocks_url}c=avatars&context=worker&context_id={$_worker->id}{/devblocks_url}?v={$_worker->updated}" style="height:1.2em;width:1.2em;border-radius:0.75em;vertical-align:middle;">
					<a class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_WORKER}" data-context-id="{$_worker->id}">{$_worker->getName()}</a>
				{/if}
			</td>
			{elseif $column == "qj_status_id"}
			<td>
				{if $result.qj_status_id == 0}
					<span class="cerb-icons cerb-icon-play"></span> Running
				{elseif $result.qj_status_id == 1}
					<span class="cerb-icons cerb-icon-pause"></span> Paused
				{elseif $result.qj_status_id == 2}
					<span class="cerb-icons cerb-icon-circle-ok"></span> Done
				{elseif $result.qj_status_id == 3}
					<span class="cerb-icons cerb-icon-ban"></span> Canceled
				{/if}
			</td>
			{elseif in_array($column, ["qj_created_at", "qj_updated_at"])}
				<td>
...
<div style="padding-top:5px;">
	...
	<div style="float:left;" id="{$view->id}_actions">
		...
		{if $active_worker->is_superuser}<button data-cerb-worklist-action-bulk="queue_job" type="button" class="action-always-show action-bulkupdate"><span class="cerb-icons cerb-icon-folder"></span> {'common.bulk_update'|devblocks_translate|lower}</button>{/if}
	</div>
</div>
{/if}

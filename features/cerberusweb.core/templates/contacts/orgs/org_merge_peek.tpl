...
{if $active_worker->hasPriv('core.addybook.org.actions.update')}
	{*<button type="button" onclick="return;if($('#formOrgPeek').validate().form()) { genericAjaxPopupPostCloseReloadView('peek','formOrgPeek', '{$view_id}', false, 'org_save'); } "><span class="cerb-sprite sprite-check"></span> {$translate->_('common.save_changes')|capitalize}</button>*}
	<button type="button" class="submit"><span class="cerb-sprite sprite-check"></span> {$translate->_('common.continue')|capitalize}</button>
{/if}
</form>

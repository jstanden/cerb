...
$$.ready(function() {
	...
	var $prompt = $panel.querySelector('.cerb-interaction-panel--form-elements--prompt');
	var $sheet = $prompt.querySelector('[data-cerb-sheet-container]');
	
	$$.disableSelection($sheet);
	
	$prompt.querySelector('.cerb-sheet').parentElement.addEventListener('click', function(e) {
		e.stopPropagation();

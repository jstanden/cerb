...
<div class="cerb-form-builder-prompt cerb-form-builder-prompt-chooser" id="{$element_id}">
	...
	<button type="button" class="chooser-abstract" data-field-name="prompts[{$var}][]" data-context="{$record_type}" {if $selection == "single"}data-single="true"{/if} {if $autocomplete == 1}data-autocomplete="{$query}"{/if} data-query="{$record_query}" data-query-required="{$record_query_required}" data-shortcuts><span class="cerb-icons cerb-icon-search"></span></button>

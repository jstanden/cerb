{if isset($message_notes.$message_id) && is_array($message_notes.$message_id)}
	{foreach from=$message_notes.$message_id item=note name=notes key=note_id}
		...
		<div class="message_note" style="margin:10px;margin-left:20px;">
			...
			<a href="javascript:;" onclick="if(confirm('Are you sure you want to permanently delete this note?')) { genericAjaxGet('','c=internal&a=commentDelete&id={$note->id|escape}');$(this).closest('div.message_note').remove(); } ">{$translate->_('common.delete')|lower}</a><br>

...
<div class="cerb-profile-toolbar">
	<form class="toolbar" action="{devblocks_url}{/devblocks_url}" onsubmit="return false;" style="margin-bottom:5px;">
		...
		<!-- Edit -->
		<button type="button" id="btnDisplaySkillsetEdit" title="{'common.edit'|devblocks_translate|capitalize}">&nbsp;<span class="cerb-sprite2 sprite-gear"></span>&nbsp;</button>
	</form>
...
<fieldset class="properties">
	<legend>{'Skillset'|devblocks_translate|capitalize}</legend>
...
$(function() {
	...
	tabOptions.active = Devblocks.getjQueryUiTabSelected('skillsetTabs');

	var tabs = $("#skillsetTabs").tabs(tabOptions);
	
	$('#btnDisplaySkillsetEdit').bind('click', function() {
		$popup = genericAjaxPopup('peek','c=internal&a=showPeekPopup&context={$page_context}&context_id={$page_context_id}',null,false,'550');
		$popup.one('skillset_save', function(event) {
...
$(document).keypress(function(event) {
	...
	switch(event.which) {
		...
		case 58:  // (0) tab cycle
			try {
				idx = event.which-49;
				$tabs = $("#skillsetTabs").tabs();
				$tabs.tabs('option', 'active', idx);
			} catch(ex) { }
			break;
		case 101:  // (E) edit
			try {
				$('#btnDisplaySkillsetEdit').click();
			} catch(ex) { }
			break;
		case 109:  // (M) macros
			try {

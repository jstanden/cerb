...
{$is_writeable = Context_PackageLibrary::isWriteableByActor($dict, $active_worker)}
...
<div id="{$div_id}">
	...
	<div style="float:left;">
		...
		<div style="margin-top:5px;">
			...
			<button type="button" class="cerb-peek-edit" data-context="{$peek_context}" data-context-id="{$dict->id}" data-edit="true" data-width="75%"><span class="glyphicons glyphicons-cogwheel"></span> {'common.edit'|devblocks_translate|capitalize}</button>
			{/if}
	...
	$popup.one('popup_open',function(event,ui) {
		...
		$popup.find('.cerb-peek-profile').click(function(e) {
			if(e.shiftKey || e.metaKey) {
				window.open('{devblocks_url}c=profiles&type=package&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}', '_blank', 'noopener');
				
			} else {
				document.location='{devblocks_url}c=profiles&type=package&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}';
			}
		});

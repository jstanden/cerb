...
<table cellpadding="0" cellspacing="0" border="0" class="worklist" width="100%" {if array_key_exists('header_color', $view->options) && $view->options.header_color}style="background-color:{$view->options.header_color};"{/if}>
	<tr>
		...
		<td nowrap="nowrap" align="right" class="title-toolbar">
			{if $active_worker->hasPriv("contexts.{$view_context}.create")}<a title="{'common.add'|devblocks_translate|capitalize}" class="minimal cerb-peek-trigger" data-context="{CerberusContexts::CONTEXT_BUCKET}" data-context-id="0" data-edit="true"><span class="cerb-icons cerb-icon-circle-plus"></span></a>{/if}
	...
	<tbody style="cursor:pointer;">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td data-column="{$column}">
				...
				<button type="button" class="cerb-peek-trigger peek" data-context="{$view_context}" data-context-id="{$result.b_id}"><span class="cerb-icons cerb-icon-new-window" style="margin-left:2px" title="{$translate->_('views.peek')}"></span></button>
			</td>
			...
			{elseif $column == "b_is_default"}
				<td data-column="{$column}">
					{if $result.$column}<span class="cerb-icons cerb-icon-circle-ok" style="vertical-align:middle;font-size:120%;"></span>{/if}
				</td>
			...
			{elseif $column == "b_reply_signing_key_id"}
				...
				<td data-column="{$column}">
					{if $signing_key}
					<a class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_GPG_PRIVATE_KEY}" data-context-id="{$result.$column}">{$signing_key->name}</a>
					{/if}
				</td>

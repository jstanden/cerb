...
<div class="toolbar">
	...
	{else}
		{if $editable}
			...
			<button type="button" onclick="genericAjaxPopup('simulate_behavior','c=internal&a=showBehaviorSimulatorPopup&trigger_id={$job->behavior_id}&context={$job->context}&context_id={$job->context_id}','reuse',false,'500');"> <span class="glyphicons glyphicons-cogwheel"></span> Simulator</button>
		...
		{else}
			<button type="button" onclick="genericAjaxPopupDestroy('peek');"><span class="glyphicons glyphicons-circle-ok" style="color:rgb(0,180,0);"></span> {'common.ok'|devblocks_translate}</button>
		{/if}
	{/if}
</div>

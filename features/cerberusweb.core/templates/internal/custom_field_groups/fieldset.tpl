...
<fieldset class="peek" id="{$fieldset_domid}">
	<legend>{$custom_field_group->name}</legend>
	<span class="cerb-sprite2 sprite-cross-circle delete" style="cursor:pointer;float:right;margin-top:-20px;display:none;"></span>
	
	{if $custom_field_group_is_new}
	<input type="hidden" name="custom_fieldset_adds[]" value="{$custom_field_group->id}">
	{else}{* We can only delete fieldsets that existed first *}
	<input type="hidden" name="custom_fieldset_deletes[]" value="">
	{/if}
	
	{include file="devblocks:cerberusweb.core::internal/custom_fields/bulk/form.tpl" bulk=false custom_fields=$custom_field_group->getCustomFields()}
</fieldset>
...
$('#{$fieldset_domid}')
	...
	.click(function() {
		...
		if($hidden.length == 0) {
			...
			$hidden.val('{$custom_field_group->id}');
		} else {

{$view_context = CerberusContexts::CONTEXT_EMAIL_SIGNATURE}
...
<table cellpadding="5" cellspacing="0" border="0" width="100%" class="worklistBody">
	...
	<tbody style="cursor:pointer;">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			{elseif $column == "e_name"}
			<td>
				<input type="checkbox" name="row_id[]" value="{$result.e_id}" style="display:none;">
				<a href="{devblocks_url}c=profiles&type=email_signature&id={$result.e_id}-{$result.e_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.e_name}</a>
				<button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.e_id}"><span class="cerb-icons cerb-icon-new-window"></span></button>
			</td>
			{elseif in_array($column, ["e_updated_at"])}
				<td>

...
<div id="abstract_custom_recordTabs">
	<ul>
...
$(function() {
	...
	tabOptions.active = Devblocks.getjQueryUiTabSelected('abstract_custom_recordTabs');

	var tabs = $("#abstract_custom_recordTabs").tabs(tabOptions);

	$('#btnProfileCard').cerbPeekTrigger();
	
	// Edit
	{if $is_writeable && $active_worker->hasPriv("contexts.{$peek_context}.update")}
	$('#btnDisplayAbstractCustomRecordEdit')
		.cerbPeekTrigger()
	...
	$(document).keypress(function(event) {
		...
		switch(event.which) {
			...
			case 58:  // (0) tab cycle
				try {
					idx = event.which-49;
					$tabs = $("#abstract_custom_recordTabs").tabs();
					$tabs.tabs('option', 'active', idx);
				} catch(ex) { }
				break;
			case 101:  // (E) edit
				try {
					$('#btnDisplayAbstractCustomRecordEdit').click();
				} catch(ex) { }
				break;
			case 109:  // (M) macros
				try {

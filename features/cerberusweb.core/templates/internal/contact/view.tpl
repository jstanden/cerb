...
<table cellpadding="0" cellspacing="0" border="0" class="worklist" width="100%" {if array_key_exists('header_color', $view->options) && $view->options.header_color}style="background-color:{$view->options.header_color};"{/if}>
	<tr>
		...
		<td nowrap="nowrap" align="right" class="title-toolbar">
			{if $active_worker->hasPriv("contexts.{$view_context}.create")}<a title="{'common.add'|devblocks_translate|capitalize}" class="minimal cerb-peek-trigger" data-context="{CerberusContexts::CONTEXT_CONTACT}" data-context-id="0"><span class="cerb-icons cerb-icon-circle-plus"></span></a>{/if}
...
<table cellpadding="1" cellspacing="0" border="0" width="100%" class="worklistBody">
	...
	<tr>
		...
		<th class="no-sort" style="text-align:center;width:40px;padding-left:0;padding-right:0;" title="{'common.photo'|devblocks_translate|capitalize}">
			<span class="cerb-icons cerb-icon-camera"></span>
		</th>
		{/if}
	...
	<tbody style="cursor:pointer;">
		<tr class="{$tableRowClass}">
			...
			<td data-column="label" colspan="{$smarty.foreach.headers.total}">
				...
				<button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.c_id}"><span class="cerb-icons cerb-icon-new-window" style="margin-left:2px" title="{$translate->_('views.peek')}"></span></button>
			</td>
		</tr>
		<tr class="{$tableRowClass}">
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td data-column="{$column}">
				{if $result.$column == 'M'}
				<span class="cerb-icons cerb-icon-gender-male"></span>
				{elseif $result.$column == 'F'}
				<span class="cerb-icons cerb-icon-gender-female"></span>
				{else}
				{/if}
			</td>
...
<div style="padding-top:5px;">
	...
	<div style="float:left;" id="{$view->id}_actions">
		...
		{if $active_worker->hasPriv("contexts.{$view_context}.update.bulk")}<button data-cerb-worklist-action-bulk="contact" type="button" class="action-always-show action-bulkupdate"><span class="cerb-icons cerb-icon-folder"></span> {'common.bulk_update'|devblocks_translate|lower}</button>{/if}

...
<div class="node {$node->node_type}" {if $node->status_id}style="opacity:0.5;"{/if}>
	...
	{if $node->node_type == 'subroutine'}
		<div class="badge badge-lightgray" style="margin:2px;">
			<a style="text-decoration:none;font-weight:bold;color:var(--cerb-color-background-contrast-50);" node_id="{$node->id}" trigger_id="{$trigger_id}">
	...
	{elseif $node->node_type == 'switch'}
		<div class="badge badge-lightgray" style="margin:2px;">
			<a style="text-decoration:none;font-weight:bold;color:rgb(68,154,220);" node_id="{$node->id}" trigger_id="{$trigger_id}">
	...
	{elseif $node->node_type == 'loop'}
		<div class="badge badge-lightgray" style="margin:2px;">
			<a style="text-decoration:none;font-weight:bold;color:var(--cerb-color-background-contrast-100);" node_id="{$node->id}" trigger_id="{$trigger_id}">
				<span style="font-weight:normal;">&#x27f3;</span> {$node->title}{if $is_writeable} &#x25be;{/if}
			</a>
		</div>
	...
	{elseif $node->node_type == 'outcome'}
		<div class="badge badge-lightgray">
			<a style="text-decoration:none;font-weight:bold;{if preg_match('#^yes($|,| )#i',$node->title)}color:rgb(0,150,0);{elseif preg_match('#^no($|,| )#i',$node->title)}color:rgb(150,0,0);{/if}" node_id="{$node->id}" trigger_id="{$trigger_id}">
				{$node->title}{if $is_writeable} &#x25be;{/if}
			</a>
		</div>
	...
	{elseif $node->node_type == 'action'}
		<div class="badge badge-lightgray" style="margin:2px;">
			<a style="text-decoration:none;font-weight:normal;font-style:italic;" node_id="{$node->id}" trigger_id="{$trigger_id}">
				{$node->title}{if $is_writeable} &#x25be;{/if}
			</a>
		</div>

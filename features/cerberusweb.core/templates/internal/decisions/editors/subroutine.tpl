<form id="frmDecisionSubroutine{$id}" onsubmit="return false;">
...
<fieldset class="delete" style="display:none;">
	...
	<button type="button" class="green" onclick="genericAjaxPost('frmDecisionSubroutine{$id}','','c=internal&a=saveDecisionDeletePopup',function() { genericAjaxPopupDestroy('node_subroutine{$id}'); genericAjaxGet('decisionTree{$trigger_id}','c=internal&a=showDecisionTree&id={$trigger_id}'); });"> {'common.yes'|devblocks_translate|capitalize}</button>
...
<form class="toolbar">
	<button type="button" onclick="genericAjaxPost('frmDecisionSubroutine{$id}','','c=internal&a=saveDecisionPopup',function() { genericAjaxPopupDestroy('node_subroutine{$id}'); genericAjaxGet('decisionTree{$trigger_id}','c=internal&a=showDecisionTree&id={$trigger_id}'); });"><span class="glyphicons glyphicons-circle-ok" style="color:rgb(0,180,0);"></span> {'common.save_changes'|devblocks_translate|capitalize}</button>

...
$('#{$btn_cfield_group_domid}')
	.each(function() {
		...
		.click(function() {
			...
			genericAjaxGet('', 'c=internal&a=handleSectionAction&section=custom_fieldsets&action=getCustomFieldSet&bulk={if !empty($bulk)}1{else}0{/if}{if $field_wrapper}&field_wrapper={$field_wrapper|escape:'url'}{/if}&id=' + cf_group_id, function(html) {

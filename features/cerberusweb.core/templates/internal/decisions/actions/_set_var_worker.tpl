...
<div style="margin-left:10px;">
	...
	{foreach from=$params.worker_id item=worker_id}
		...
		<li>{$context_worker->getName()}<input type="hidden" name="{$namePrefix}[worker_id][]" value="{$context_worker->id}"><a onclick="$(this).parent().remove();"><span class="glyphicons glyphicons-circle-remove"></span></a></li>
		{/if}
	{/foreach}
	{/if}
	</ul>
...
<div style="margin-left:10px;">
	...
	{foreach from=$params.group_id item=group_id}
		...
		<li>{$context_group->name}<input type="hidden" name="{$namePrefix}[group_id][]" value="{$context_group->id}"><a onclick="$(this).parent().remove();"><span class="glyphicons glyphicons-circle-remove"></span></a></li>
		{/if}
	{/foreach}
	{/if}
	</ul>

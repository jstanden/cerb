...
{foreach from=$params.$param_name item=worker_id}
	{if !is_numeric($worker_id) && isset($values_to_contexts.$worker_id)}
		...
		{if !empty($var_data)}
		<li>{$var_data.label}<input type="hidden" name="{$namePrefix}[{$param_name}]{if !$single}[]{/if}" value="{$worker_id}"><a onclick="$(this).parent().remove();"><span class="glyphicons glyphicons-circle-remove"></span></a></li>
		{/if}

{$view_context = CerberusContexts::CONTEXT_GPG_PUBLIC_KEY}
	...
	<tbody style="cursor:pointer;">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td>
				...
				<a href="{devblocks_url}c=profiles&type=gpg_public_key&id={$result.g_id}-{$result.g_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.g_name}</a>
				<button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.g_id}"><span class="cerb-icons cerb-icon-new-window"></span></button>
			</td>
			{elseif in_array($column, ["g_expires_at", "g_updated_at"])}
				<td>

...
{$is_writeable = Context_GpgPublicKey::isWriteableByActor($dict, $active_worker)}
...
{if $keyinfo && is_array($keyinfo)}
<fieldset class="peek">
	<legend>UIDs</legend>
	
	<ul style="margin-top:0;margin-bottom:5px;">
	{foreach from=$keyinfo item=key}
	{foreach from=$key.uids item=uid}
		<li>{$uid.uid}</li>
	{/foreach}
	{/foreach}
	</ul>
...
<fieldset class="peek">
	<legend>Subkeys</legend>
	
	<table cellspacing="0" cellpadding="0" width="100%">
	{foreach from=$keyinfo item=key}
	{foreach from=$key.subkeys item=subkey}
		<tr>
			<td>
				{if $subkey.expired || $subkey.disabled || $subkey.revoked || $subkey.invalid}
				<b><strike>{$subkey.keyid}</strike></b> 
				{else}
				<b>{$subkey.keyid}</b> 
				{/if}
			</td>
			<td>
				{$subkey.timestamp|devblocks_date:'Y-m-d'}
			</td>
			<td>
				[{if $subkey.can_sign}S{/if}{if $subkey.can_encrypt}E{/if}]
			</td>
			<td>
				{if $subkey.expires}
					{if $subkey.expires < time()}
					<i>{'common.expired'|devblocks_translate|lower} {$subkey.expires|devblocks_date:'Y-m-d'}</i>
					{else}
					{'common.expires'|devblocks_translate|lower} {$subkey.expires|devblocks_date:'Y-m-d'}
					{/if}
				{else}
				never expires
				{/if}
	...
	$popup.one('popup_open',function(event,ui) {
		...
		$popup.find('.cerb-peek-profile').click(function(e) {
			if(e.shiftKey || e.metaKey) {
				window.open('{devblocks_url}c=profiles&type=gpg_public_key&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}', '_blank', 'noopener');
				
			} else {
				document.location='{devblocks_url}c=profiles&type=gpg_public_key&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}';
			}
		});

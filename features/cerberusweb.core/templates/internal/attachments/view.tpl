{$view_context = CerberusContexts::CONTEXT_ATTACHMENT}
	...
	<tbody style="cursor:pointer;">
		<tr class="{$tableRowClass}">
			<td data-column="label" colspan="{$smarty.foreach.headers.total}">
				...
				<a href="{devblocks_url}c=profiles&type=attachment&id={$result.a_id}-{$result.a_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.a_name}</a>
				<button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.a_id}" data-profile-url="{devblocks_url}c=files&id={$result.a_id}&name={$result.a_name|devblocks_permalink}{/devblocks_url}" data-width="55%"><span class="glyphicons glyphicons-new-window-alt"></span></button>
			</td>
		</tr>
		<tr class="{$tableRowClass}">
		{foreach from=$view->view_columns item=column name=columns}
			...
			{elseif in_array($column, ["a_updated"])}
				<td>
			...
			{elseif $column == 'a_storage_size'}
				<td data-column="{$column}">{$result.$column|devblocks_prettybytes}</td>
			{else}
...
<div style="padding-top:5px;">
	...
	<div style="float:left;" id="{$view->id}_actions">
		...
		{if $active_worker->is_superuser && $active_worker->hasPriv("contexts.{$view_context}.update.bulk")}<button type="button" class="action-always-show action-bulkupdate" onclick="genericAjaxPopup('peek','c=profiles&a=invoke&module=attachment&action=showBulkPopup&view_id={$view->id}&ids=' + Devblocks.getFormEnabledCheckboxValues('viewForm{$view->id}','row_id[]'),null,false,'50%');"><span class="glyphicons glyphicons-folder-closed"></span> {'common.bulk_update'|devblocks_translate|lower}</button>{/if}
	</div>
</div>
{/if}

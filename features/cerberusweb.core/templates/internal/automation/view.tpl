...
<table cellpadding="0" cellspacing="0" border="0" class="worklist" width="100%" {if array_key_exists('header_color', $view->options) && $view->options.header_color}style="background-color:{$view->options.header_color};"{/if}>
	<tr>
		...
		<td nowrap="nowrap" align="right" class="title-toolbar">
			{if $active_worker->is_superuser && $active_worker->hasPriv("contexts.{$view_context}.create")}<a title="{'common.add'|devblocks_translate|capitalize}" class="minimal peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="0" data-width="90%"><span class="cerb-icons cerb-icon-circle-plus"></span></a>{/if}
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			{elseif $column == "*_sparkline"}
				<td data-column="{$column}" style="width:160px;">
					<div class="cerb-ui-sparkchart" data-cerb-spark="{$result.a_id}" style="min-width:140px;"></div>
				</td>
			{else}
...
$(function() {
	...
	{include file="devblocks:cerberusweb.core::internal/views/sparkline_loader.tpl" spark_module='automation' spark_action='viewSparklinesJson' spark_view_id=$view->id}
	{/if}

...
<div id="comment{$comment->id}">
	<div class="block">
		<h3 style="display:inline;"><span style="background-color:rgb(232,242,254);color:rgb(71,133,210);">{$translate->_('common.comment')|lower}</span> <a href="javascript:;" onclick="genericAjaxPopup('peek','c=contacts&a=showAddressPeek&address_id={$comment_address->id}', null, false, '500');" title="{$comment_address->email|escape}">{if empty($comment_address->first_name) && empty($comment_address->last_name)}&lt;{$comment_address->email|escape}&gt;{else}{$comment_address->getName()}{/if}</a></h3> &nbsp;

{$view_context = CerberusContexts::CONTEXT_SKILL}
	...
	<tbody style="cursor:pointer;">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td>
				...
				<a href="{devblocks_url}c=profiles&type=skill&id={$result.s_id}-{$result.s_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.s_name}</a>
				<button type="button" class="peek" style="visibility:hidden;padding:1px;margin:0px 5px;" onclick="genericAjaxPopup('peek','c=internal&a=showPeekPopup&context={$view_context}&context_id={$result.s_id}&view_id={$view->id}',null,false,'550');"><span class="cerb-sprite2 sprite-document-search-result" style="margin-left:2px" title="{$translate->_('views.peek')}"></span></button>
			</td>
			{elseif in_array($column, ["s_created_at","s_updated_at"])}
				<td title="{$result.$column|devblocks_date}">
					{if !empty($result.$column)}
						{$result.$column|devblocks_prettytime}&nbsp;
					{/if}
				</td>
			{elseif $column == "s_skillset_id"}
			<td>
				{$skillset_id = $result.$column}
				{$skillset = $skillsets.$skillset_id}
				{if $skillset}
				<a href="{devblocks_url}c=profiles&type=skillset&id={$skillset->id}-{$skillset->name|devblocks_permalink}{/devblocks_url}">{$skillset->name}</a>
				{/if}
			</td>
			{else}
				<td>{$result.$column}</td>
...
<div style="padding-top:5px;">
	...
	<div style="float:left;" id="{$view->id}_actions">
		...
		{*
		{if $active_worker->hasPriv('calls.actions.update_all')}<button type="button" class="action-always-show action-bulkupdate" onclick="genericAjaxPopup('peek','c=profiles&a=handleSectionAction&section=skill&action=showBulkPanel&view_id={$view->id}&ids=' + Devblocks.getFormEnabledCheckboxValues('viewForm{$view->id}','row_id[]'),null,false,'500');"><span class="cerb-sprite2 sprite-folder-gear"></span> {'common.bulk_update'|devblocks_translate|lower}</button>{/if}
		*}
	</div>
	{/if}
</div>

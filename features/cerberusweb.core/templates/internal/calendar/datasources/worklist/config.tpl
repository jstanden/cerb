...
<select class="context">
	...
	<option value="{$context_mft->id}" {if $calendar->params.worklist_model.context==$context_mft->id}selected="selected"{/if}>{$context_mft->name}</option>
	{/foreach}
</select>
...
<input type="hidden" name="params[worklist_model_json]" value="{$calendar->params.worklist_model|json_encode}" class="model">
...
<select name="params[field_start_date]" class="field_start_date">
	...
	{foreach from=$ctx_fields item=field}
		{if !empty($field->db_label)}
			...
			<option value="{$field->token}" class="{if $field->type == Model_CustomField::TYPE_DATE}date{else}{/if}" {if $calendar->params.field_start_date==$field->token}selected="selected"{/if}>{$field->db_label|lower}</option>
			{/if}
		{/if}
	{/foreach}
	{/if}
...
<input type="hidden" name="params[color]" value="{$calendar->params.color|default:'#A0D95B'}" style="width:100%;" class="color-picker">
</div>

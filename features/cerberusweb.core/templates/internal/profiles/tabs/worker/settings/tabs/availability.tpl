...
<fieldset class="peek">
	...
	<ul class="bubbles chooser-container">
		...
		{if $calendar}
			<li><input type="hidden" name="availability_calendar_id" value="{$calendar->id}"><a class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_CALENDAR}" data-context-id="{$calendar->id}">{$calendar->name}</a></li>
		{/if}
	</ul>
</fieldset>

...
{$is_writeable = Context_ProfileTab::isWriteableByActor($dict, $active_worker)}
	...
	$popup.one('popup_open',function(event,ui) {
		...
		$popup.find('.cerb-peek-profile').click(function(e) {
			if(e.shiftKey || e.metaKey) {
				window.open('{devblocks_url}c=profiles&type=profile_tab&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}', '_blank', 'noopener');
				
			} else {
				document.location='{devblocks_url}c=profiles&type=profile_tab&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}';
			}
		});

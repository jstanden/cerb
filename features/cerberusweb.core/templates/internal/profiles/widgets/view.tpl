{$view_context = CerberusContexts::CONTEXT_PROFILE_WIDGET}
	...
	<tbody style="cursor:pointer;">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td>
				...
				<a href="{devblocks_url}c=profiles&type=profile_widget&id={$result.p_id}-{$result.p_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.p_name}</a>
			...
			{elseif $column == "p_profile_tab_id"}
			<td>
				{$profile_tab = $profile_tabs.{$result.$column}}
				{if $profile_tab}
					<a class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_PROFILE_TAB}" data-context-id="{$profile_tab->id}">{$profile_tab->name}</a>
				{else}
					{$result.$column}
				{/if}
			</td>
			...
			<td>
				{$display_ext = Extension_ProfileWidget::get($result.$column)}
				{if $display_ext}

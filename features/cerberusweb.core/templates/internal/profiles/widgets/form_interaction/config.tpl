<div id="widget{$widget->id}Config" style="margin-top:10px;">
	<fieldset class="peek">
		...
		<div>
			<textarea name="params[interactions_yaml]" class="cerb-code-editor" data-editor-mode="ace/mode/yaml" style="width:100%;">{$widget->extension_params.interactions_yaml}</textarea>
		</div>
		...
		.on('cerb-chooser-selected', function(e) {
			...
			genericAjaxGet(null, 'c=ui&a=dataQuery&q=' + encodeURIComponent('type:worklist.records of:behaviors query:(id:[' + e.values.join(',') + ']) format:dictionaries'), function(json) {
				...
				for(id in json.data) {
					...
					// [TODO] Default input examples by type

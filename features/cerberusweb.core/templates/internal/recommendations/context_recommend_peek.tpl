...
<form id="{$div_uniqid}" style="margin-bottom:10px;" action="{devblocks_url}{/devblocks_url}" method="post">
	...
	<input type="hidden" name="action" value="saveContextRecommendationsPopupJson">
...
$(function() {
	...
	$popup.one('popup_open', function(event,ui) {
		$popup
			.dialog('option','title',"{'common.recommendations'|devblocks_translate|capitalize|escape:'javascript' nofilter}")
			;
		...
		$btn.click(function() {
			...
			genericAjaxPost($frm, '', null, function(json) {
				...
				if(undefined != json.count && undefined != json.has_active_worker) {
					var event = jQuery.Event('recommendations_save');
					event.recommendations_count = json.count;
					event.recommendations_include_worker = json.has_active_worker;
					$popup.trigger(event);
				}

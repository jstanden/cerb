			...
			{elseif $column == "c_calendar_id"}
				<td data-column="{$column}">
					...
					{if $calendar instanceof Model_Calendar}
						...
						<a href="javascript:;" onclick="genericAjaxPopup('peek','c=internal&a=showPeekPopup&context={CerberusContexts::CONTEXT_CALENDAR}&context_id={$calendar->id}&view_id={$view->id}',null,false,'50%');">{$calendar->name}</a>
						{else}
						{$calendar->name}
						{/if}
					{/if}

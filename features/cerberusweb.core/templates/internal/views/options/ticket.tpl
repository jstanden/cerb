<div>
	<label><input type="checkbox" name="view_options[disable_watchers]" value="1" {if $view->options.disable_watchers}checked="checked"{/if}> {'ticket-option-hide-watcher'|devblocks_translate}Hide watchers column</label>
</div>

<fieldset class="peek peek-noborder black" style="margin-top:10px;">
	<legend>{'common.compose'|devblocks_translate|capitalize}</legend>
	
	{'common.worklist.compose.sender'|devblocks_translate}When composing mail from this worklist, set the sender to:

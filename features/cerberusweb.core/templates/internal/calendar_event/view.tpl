...
<table cellpadding="0" cellspacing="0" border="0" class="worklist" width="100%" {if array_key_exists('header_color', $view->options) && $view->options.header_color}style="background-color:{$view->options.header_color};"{/if}>
	<tr>
		...
		<td nowrap="nowrap" align="right" class="title-toolbar">
			{if $active_worker->hasPriv("contexts.{$view_context}.create")}<a title="{'common.add'|devblocks_translate|capitalize}" class="minimal cerb-peek-trigger" data-context="{CerberusContexts::CONTEXT_CALENDAR_EVENT}" data-context-id="0" data-edit="true"><span class="cerb-icons cerb-icon-circle-plus"></span></a>{/if}
	...
	<tbody style="cursor:pointer;">
		<tr class="{$tableRowClass}">
			<td data-column="label" colspan="{$smarty.foreach.headers.total}">
				...
				<button type="button" class="peek cerb-peek-trigger" data-context="{CerberusContexts::CONTEXT_CALENDAR_EVENT}" data-context-id="{$result.c_id}"><span class="cerb-icons cerb-icon-new-window"></span></button>
			</td>
		</tr>
		<tr class="{$tableRowClass}">
			...
			{elseif $column=="c_calendar_id"}
				<td data-column="{$column}">
					...
					{if isset($calendars.$calendar_id)}
						<a class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_CALENDAR}" data-context-id="{$calendar_id}">{$calendars.$calendar_id->name}</a>
					{/if}
				</td>

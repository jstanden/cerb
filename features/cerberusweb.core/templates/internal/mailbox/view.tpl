		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			{elseif $column == "*_sparkline"}
				<td data-column="{$column}" style="width:160px;">
					<div class="cerb-ui-sparkchart" data-cerb-spark="{$result.p_id}" style="min-width:140px;"></div>
				</td>
			{else}
				<td data-column="{$column}">{$result.$column}</td>
			{/if}
...
$(function() {
	...
	{include file="devblocks:cerberusweb.core::internal/views/sparkline_loader.tpl" spark_module='mailbox' spark_action='viewSparklinesJson' spark_view_id=$view->id}
	{/if}

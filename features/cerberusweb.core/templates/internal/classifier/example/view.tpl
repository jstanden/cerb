...
<table cellpadding="2" cellspacing="0" border="0" width="100%" class="worklistBody">
	...
	{else}
		{$tableRowClass = "odd"}
	{/if}
	<tbody style="cursor:pointer;" class="cerb-expression-editor">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			{elseif $column == "c_class_id"}
			<td>
				{$class = $classes.{$result.$column}}
				{if $class}
					<a href="javascript:;" class="cerb-peek-trigger" data-context="{CerberusContexts::CONTEXT_CLASSIFIER_CLASS}" data-context-id="{$class->id}">{$class->name}</a>
				{/if}
			</td>
			...
			<td>
				...
				{if $classifier}
					<a href="javascript:;" class="cerb-peek-trigger" data-context="{CerberusContexts::CONTEXT_CLASSIFIER}" data-context-id="{$classifier->id}">{$classifier->name}</a>
				{/if}
			</td>

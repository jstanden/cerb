...
{$is_writeable = Context_ClassifierExample::isWriteableByActor($dict, $active_worker)}

<div id="{$div_id}">
	<div style="float:left;">
		<h1 style="color:inherit;" class="cerb-expression-editor">
			<span class="expression">
				{$dict->_label|devblocks_rangy_deserialize nofilter}
			</span>
		</h1>
	...
	$popup.one('popup_open',function(event,ui) {
		...
		$popup.find('.cerb-peek-profile').click(function(e) {
			if(e.shiftKey || e.metaKey) {
				window.open('{devblocks_url}c=profiles&type=classifier_example&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}', '_blank', 'noopener');
				
			} else {
				document.location='{devblocks_url}c=profiles&type=classifier_example&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}';
			}
		});
	});
});

{$div_id = "peek{uniqid()}"}
{$peek_context = CerberusContexts::CONTEXT_VIRTUAL_ATTENDANT}
...
$(function() {
	...
	$popup.one('popup_open',function(event,ui) {
		...
		$popup.find('.cerb-peek-profile').click(function(e) {
			if(e.metaKey) {
				window.open('{devblocks_url}c=profiles&type=virtual_attendant&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}', '_blank');
				
			} else {
				document.location='{devblocks_url}c=profiles&type=virtual_attendant&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}';
			}
		});

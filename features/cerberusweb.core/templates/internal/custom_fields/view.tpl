{$view_context = CerberusContexts::CONTEXT_CUSTOM_FIELD}
	...
	<tbody style="cursor:pointer;">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td>
				...
				<a href="{devblocks_url}c=profiles&type=custom_field&id={$result.c_id}-{$result.c_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.c_name}</a>
				<button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.c_id}"><span class="cerb-icons cerb-icon-new-window"></span></button>
			</td>
			{elseif $column == "c_context"}
			<td>
				{$context_mft = $context_mfts.{$result.$column}} 
				{if $context_mft}
					{$context_mft->name}
				{else}
					{$result.$column}
			...
			{elseif $column == "c_custom_fieldset_id"}
			<td>
				{$custom_fieldset = $custom_fieldsets.{$result.$column}} 
				{if $custom_fieldset}
					<a class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_CUSTOM_FIELDSET}" data-context-id="{$custom_fieldset->id}">{$custom_fieldset->name}</a>
				{else}
					{if $result.$column}
						{$result.$column}
					{/if}
			...
			<td>
				{$type = $custom_field_types.{$result.$column}}
				{if $type}
					{$type}
				{else}
					{$result.$column}

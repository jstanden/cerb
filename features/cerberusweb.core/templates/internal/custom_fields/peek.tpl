...
{$is_writeable = Context_CustomField::isWriteableByActor($dict, $active_worker)}
...
<fieldset class="peek">
	...
	<div class="cerb-properties-grid" data-column-width="100">
		...
		{foreach from=$properties item=k name=props}
			...
			<div>
			{if $k == 'type'}
				<label>{'common.type'|devblocks_translate|capitalize}</label>
				{$field_types = Model_CustomField::getTypes()}
				{$field_type = $field_types[$dict->$k]}
				{if $field_type}
					{$field_type}
				{else}
					{$dict->$k}
...
$(function() {
	...
	$popup.one('popup_open',function(event,ui) {
		$popup.dialog('option','title', "{'common.custom_field'|devblocks_translate|capitalize|escape:'javascript' nofilter}");
		...
		$popup.find('.cerb-peek-profile').click(function(e) {
			if(e.shiftKey || e.metaKey) {
				window.open('{devblocks_url}c=profiles&type=custom_field&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}', '_blank', 'noopener');
				
			} else {
				document.location='{devblocks_url}c=profiles&type=custom_field&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}';
			}
		});

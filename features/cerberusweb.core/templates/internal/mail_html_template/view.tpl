...
<table cellpadding="0" cellspacing="0" border="0" class="worklist" width="100%" {if array_key_exists('header_color', $view->options) && $view->options.header_color}style="background-color:{$view->options.header_color};"{/if}>
	<tr>
		...
		<td nowrap="nowrap" align="right" class="title-toolbar">
			{if $active_worker->is_superuser && $active_worker->hasPriv("contexts.{$view_context}.create")}<a class="cerb-peek-trigger" title="{'common.add'|devblocks_translate|capitalize}" class="minimal" data-context="{CerberusContexts::CONTEXT_MAIL_HTML_TEMPLATE}" data-context-id="0"><span class="cerb-icons cerb-icon-circle-plus"></span></a>{/if}
	...
	<tbody style="cursor:pointer;">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td data-column="{$column}">
				...
				<button type="button" class="peek cerb-peek-trigger" data-context="{CerberusContexts::CONTEXT_MAIL_HTML_TEMPLATE}" data-context-id="{$result.m_id}"><span class="cerb-icons cerb-icon-new-window" style="margin-left:2px" title="{$translate->_('views.peek')}"></span></button>
			</td>
			{elseif $column == "m_signature_id"}
				{$email_signature = $email_signatures.{$result.$column}}
				<td data-column="{$column}">
					{if $email_signature}
						<a class="cerb-peek-trigger" data-context="{CerberusContexts::CONTEXT_EMAIL_SIGNATURE}" data-context-id="{$result.$column}">{$email_signature->name}</a>
					{/if}
				</td>

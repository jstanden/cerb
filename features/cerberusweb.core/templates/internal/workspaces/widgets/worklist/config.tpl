		...
		<div class="cerb-ui-form">
			<div class="cerb-ui-form--field">
				...
				<select name="params[context]" data-cerb-worklist-context>
					...
					<option value="{$context_mft->id}" data-cerb-ui-icon="{$context_mft->params.icon|default:'collection'}" {if $widget->params.context == $context_mft->id}selected="selected"{/if}>{$context_mft->name}</option>
					{/foreach}
				</select>
			</div>
			...
			<div class="cerb-ui-form--field">
				...
				<div class="cerb-ui-searchquery placeholders" data-cerb-searchquery>
					...
					<div class="cerb-ui-searchquery--field">
						...
						<textarea name="params[query_required]" class="cerb-ui-searchquery--input" rows="1" autocomplete="off" spellcheck="false">{$widget->params.query_required}</textarea>
			...
			<div class="cerb-ui-form--field">
				...
				<div class="cerb-ui-searchquery placeholders" data-cerb-searchquery>
					...
					<div class="cerb-ui-searchquery--field">
						<div class="cerb-ui-searchquery--highlight" aria-hidden="true"></div>
						<textarea name="params[query]" class="cerb-ui-searchquery--input" rows="1" autocomplete="off" spellcheck="false">{$widget->params.query}</textarea>
						<span class="cerb-ui-searchquery--caret-anchor"></span>
					</div>
					<div class="cerb-ui-searchquery--right">
						<a data-action="autocomplete" style="cursor:pointer;color:var(--cerb-color-background-contrast-150);" title="Suggestions (Ctrl/⌘+Space)"><span class="cerb-icons cerb-icon-autocomplete"></span></a>
			...
			<div class="cerb-ui-form--row">
				<div class="cerb-ui-form--field">
					...
					<input type="text" name="params[render_limit]" value="{$widget->params.render_limit|default:5}" class="placeholders" style="width:6em;" autocomplete="off" spellcheck="false">
				</div>

				<div class="cerb-ui-form--field">
					<label class="cerb-ui-form--label">{'common.color'|devblocks_translate|capitalize}</label>
					<div><input type="text" name="params[header_color]" value="{$widget->params.header_color|default:'#6a87db'}" class="color-picker"></div>
				</div>
			</div>
...
$(function() {
	...
	// change event still drives the query editors + columns list below.
	...
	if(window.CerbUI && CerbUI.SearchQuery) {
		$config.find('.cerb-ui-searchquery[data-cerb-searchquery]').each(function() {
			var sq = new CerbUI.SearchQuery(this, {
				onAutocomplete: CerbUI.SearchQuery.queryFieldSource('{$widget->params.context}'),

{$view_context = CerberusContexts::CONTEXT_WORKSPACE_WIDGET}
...
<table cellpadding="1" cellspacing="0" border="0" width="100%" class="worklistBody">
	...
	{* Bulk lazy load tabs *}
	{$object_tabs = []}
	{if in_array(SearchFields_WorkspaceWidget::WORKSPACE_TAB_ID, $view->view_columns)}
	...
	<tbody style="cursor:pointer;">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td>
				...
				<a href="{devblocks_url}c=profiles&type=workspace_widget&id={$result.w_id}-{$result.w_label|devblocks_permalink}{/devblocks_url}" class="subject">{$result.w_label}</a>
			...
			{elseif $column == "w_extension_id"}
				<td>
					{if $result.$column}
						{$widget_extension = Extension_WorkspaceWidget::get($result.$column)}
			...
			{elseif $column == "w_workspace_tab_id"}
				<td>
					...
					{if $object_tabs[$workspace_tab_id]}
						{$workspace_tab = $object_tabs[$workspace_tab_id]}
						<a href="javascript:;" class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_WORKSPACE_TAB}" data-context-id="{$workspace_tab->id}">{$workspace_tab->name}</a>
					{/if}
				</td>

{$view_context = CerberusContexts::CONTEXT_WORKSPACE_TAB}
	...
	<tbody style="cursor:pointer;">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td>
				...
				<a href="{devblocks_url}c=profiles&type=workspace_tab&id={$result.w_id}-{$result.w_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.w_name}</a>
			...
			{elseif in_array($column,["w_workspace_page_id"])}
				<td data-column="{$column}">
					{$workspace_page_id = $result.$column}
					{if $object_pages[$workspace_page_id]}
						{$workspace_page = $object_pages[$workspace_page_id]}
						<a href="javascript:;" class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_WORKSPACE_PAGE}" data-context-id="{$workspace_page->id}">{$workspace_page->name}</a>
					{/if}
				</td>
			{elseif in_array($column,["w_extension_id"])}
				<td data-column="{$column}">
					{$tab_extension = $tab_extensions[$result.$column]}

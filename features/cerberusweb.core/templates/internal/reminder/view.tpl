{$view_context = CerberusContexts::CONTEXT_REMINDER}
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			{elseif $column == "r_name"}
				<td>
					...
					{if $result.r_is_closed}<span class="cerb-icons cerb-icon-circle-ok"></span>{/if}
			...
			{elseif in_array($column, ["r_worker_id"])}
				<td>
					{if !empty($result.$column)}
						{$worker = DAO_Worker::get($result.r_worker_id)}
						{if $worker}
						<img class="cerb-avatar" src="{devblocks_url}c=avatars&context=worker&context_id={$worker->id}{/devblocks_url}?v={$worker->updated}">
						<a class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_WORKER}" data-context-id="{$worker->id}">{$worker->getName()}</a>
						{/if}
					{/if}
				</td>
			{elseif in_array($column, ["r_remind_at", "r_updated_at"])}
				<td>
					{if !empty($result.$column)}

...
{$is_writeable = Context_Reminder::isWriteableByActor($dict, $active_worker)}

<div id="{$div_id}">
	
	<div style="float:left;">
		<h1>
			{if $dict->is_closed}<span class="glyphicons glyphicons-circle-ok" style="vertical-align:middle;color:rgb(75,75,75);"></span>{/if}
			{$dict->_label}
		</h1>
	...
	$popup.one('popup_open',function(event,ui) {
		...
		$popup.find('.cerb-peek-profile').click(function(e) {
			if(e.shiftKey || e.metaKey) {
				window.open('{devblocks_url}c=profiles&type=reminder&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}', '_blank', 'noopener');
				
			} else {
				document.location='{devblocks_url}c=profiles&type=reminder&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}';
			}
		});

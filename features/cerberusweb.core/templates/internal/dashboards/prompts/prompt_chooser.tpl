...
<div style="display:inline-block;vertical-align:middle;">
	<div id="{$uniqid}" class="bubble cerb-filter-editor" style="padding:5px;display:block;">
		...
		<div>
			<button type="button" class="cerb-chooser-prompt" data-field-name="prompts[{$prompt.placeholder}][]" data-context="{$context_mft->id}" {if $prompt.params.query}data-query="{$prompt.params.query}"{/if} {if $prompt.params.single}data-single="true"{/if}><span class="cerb-icons cerb-icon-search"></span></button>

...
<div class="cerb-profile-toolbar">
	<form class="toolbar" action="{devblocks_url}{/devblocks_url}" onsubmit="return false;" style="margin-bottom:5px;">
		...
		{*
		{if $is_writeable}
		{devblocks_url assign=return_url full=true}c=profiles&type=snippet&id={$page_context_id}-{$snippet->name|devblocks_permalink}{/devblocks_url}
		{include file="devblocks:cerberusweb.core::internal/macros/display/button.tpl" context=$page_context context_id=$page_context_id macro_event="event.macro.snippet" return_url=$return_url}
		{/if}
		*}
...
<fieldset class="properties">
	<legend>{'common.snippet'|devblocks_translate|capitalize}</legend>
...
<div id="snippetTabs">
	<ul>
		{$tabs = [content, activity, comments]}

		<li><a href="{devblocks_url}ajax.php?c=profiles&a=handleSectionAction&section=snippet&action=showTabContent&point={$point}&context_id={$page_context_id}{/devblocks_url}">{'common.content'|devblocks_translate|capitalize}</a></li>
	...
	$(document).keypress(function(event) {
		...
		switch(event.which) {
			...
			case 58:  // (0) tab cycle
				try {
					idx = event.which-49;
					$tabs = $("#snippetTabs").tabs();
					$tabs.tabs('option', 'active', idx);
				} catch(ex) { }
				break;
			case 101:  // (E) edit
				try {
					$('#btnDisplaySnippetEdit').click();
				} catch(ex) { }
				break;
			case 109:  // (M) macros
				try {

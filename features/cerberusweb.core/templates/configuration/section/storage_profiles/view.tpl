		...
		*}
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td data-column="{$column}" style="padding:5px;">
				...
				<a href="javascript:;" onclick="genericAjaxPopup('peek','c=config&a=invoke&module=storage_profiles&action=showStorageProfilePeek&id={$result.d_id}&view_id={$view->id|escape:'url'}',null,false,'50%');" class="subject">{$result.d_name}</a>
			</td>
			{else}

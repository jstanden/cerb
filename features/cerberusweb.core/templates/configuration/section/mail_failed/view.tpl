...
<table cellpadding="0" cellspacing="0" border="0" class="worklist" width="100%">
	<tr>
		...
		<td nowrap="nowrap" align="right">
			{*
			<a href="javascript:;" title="{'common.add'|devblocks_translate|capitalize}" class="minimal" onclick="genericAjaxPopup('peek','c=internal&a=showPeekPopup&context={$view_context}&context_id=0&view_id={$view->id}&owner_context={$owner_context}&owner_context_id={$owner_context_id}',null,false,'500');"><span class="cerb-sprite2 sprite-plus-circle-frame"></span></a>
	...
	<tbody style="cursor:pointer;">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td>
				...
				<button type="button" class="peek" style="visibility:hidden;padding:1px;margin:0px 5px;" onclick="genericAjaxPopup('peek','c=config&a=handleSectionAction&section=mail_failed&action=showPeekPopup&file={$result.mf_name}&view_id={$view->id}',null,false,'550');"><span class="cerb-sprite2 sprite-document-search-result" style="margin-left:2px" title="{$translate->_('views.peek')}"></span></button>
			</td>

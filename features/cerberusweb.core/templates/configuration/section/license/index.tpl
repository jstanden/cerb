	...
	{else}
		...
		{else}
			...
			<div style="margin-top:5px;">
				<button type="button" onclick="$(this).parent().fadeOut();$('#frmLicense').fadeIn().find('input:text:first').focus();"><span class="cerb-sprite sprite-add"></span> Update License</button>
			</div>
		{/if}
	{/if}
</fieldset>
...
<fieldset>
	...
	<div class="delete_confirm" style="display:none;">
		<div class="ui-widget">
			<div class="ui-state-highlight ui-corner-all" style="margin:10px;padding:5px;display:inline-block;">
				...
				<button type="button" onclick="$frm=$(this.form);$frm.find('input:hidden[name=do_delete]').val('1');$frm.find('BUTTON.submit').click();"><span class="cerb-sprite sprite-check"></span> Yes</button>
				<button type="button" onclick="$(this).closest('div.delete_confirm').hide();"><span class="cerb-sprite sprite-forbidden"></span> No</button>
			</div>
		</div>
	</div>
	...
	{if $we_trust_you->key}<button type="button" class="delete"><span class="cerb-sprite sprite-forbidden"></span> Remove License</button>{/if}

...
<table cellpadding="1" cellspacing="0" border="0" width="100%" class="worklistBody">
	...
	{/if}
		...
		<tr class="{$tableRowClass}" id="{$rowIdPrefix}" onmouseover="$(this).addClass('hover');" onmouseout="$(this).removeClass('hover');" onclick="if(getEventTarget(event)=='TD') checkAll('{$rowIdPrefix}_s');">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td><a href="javascript:;" onclick="genericAjaxPanel('c=config&a=showWorkerPeek&id={$result.w_id}&view_id={$view->id|escape:'url'}',null,false,'550');" class="subject" style="{if $result.w_is_disabled}color:rgb(120,0,0);font-style:italic;{/if}">{$result.$column}</a>&nbsp;</td>

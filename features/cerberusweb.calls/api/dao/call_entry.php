			...
			switch($param_key) {
				case SearchFields_CallEntry::VIRTUAL_WATCHERS:
					...
					if(empty($param->value)) { // empty
						$join_sql .= "LEFT JOIN context_link AS context_watcher ON (context_watcher.from_context = 'cerberusweb.contexts.call' AND context_watcher.from_context_id = c.id AND context_watcher.to_context = 'cerberusweb.contexts.worker') ";
						$where_sql .= "AND context_watcher.to_context_id IS NULL ";
					} else {
						$join_sql .= sprintf("INNER JOIN context_link AS context_watcher ON (context_watcher.from_context = 'cerberusweb.contexts.call' AND context_watcher.from_context_id = c.id AND context_watcher.to_context = 'cerberusweb.contexts.worker' AND context_watcher.to_context_id IN (%s)) ",
							implode(',', $param->value)
						);
					}
					break;

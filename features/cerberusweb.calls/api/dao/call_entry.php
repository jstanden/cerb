...
class DAO_CallEntry extends C4_ORMHelper {
	...
	public static function getSearchQueryComponents($columns, $params, $sortBy=null, $sortAsc=null) {
		...
		foreach($params as $param) {
			...
			switch($param_key) {
				case SearchFields_CallEntry::VIRTUAL_WORKERS:
					$has_multiple_values = true;
					if(empty($param->value)) { // empty
						$join_sql .= "LEFT JOIN context_link AS context_owner ON (context_owner.from_context = 'cerberusweb.contexts.call' AND context_owner.from_context_id = c.id AND context_owner.to_context = 'cerberusweb.contexts.worker') ";
						$where_sql .= "AND context_owner.to_context_id IS NULL ";
					} else {
						$join_sql .= sprintf("INNER JOIN context_link AS context_owner ON (context_owner.from_context = 'cerberusweb.contexts.call' AND context_owner.from_context_id = c.id AND context_owner.to_context = 'cerberusweb.contexts.worker' AND context_owner.to_context_id IN (%s)) ",
							implode(',', $param->value)
						);
					}
					break;
...
class View_CallEntry extends C4_AbstractView {
	...
	function __construct() {
		...
		$this->addColumnsHidden(array(
			...
			SearchFields_CallEntry::VIRTUAL_WORKERS,
		));

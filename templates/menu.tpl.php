	...
	<tr>
		{foreach from=$pages item=m}
			{if !empty($m->manifest->params.menutitle)}
				<td width="0%" nowrap="nowrap" {if $page->id==$m->id}id="headerMenuSelected"{/if}><img src="{devblocks_url}c=resource&p=cerberusweb.core&f=images/spacer.gif{/devblocks_url}" width="10" height="1"><a href="{$m->getLink()}">{$m->manifest->params.menutitle|lower}</a><img src="{devblocks_url}c=resource&p=cerberusweb.core&f=images/spacer.gif{/devblocks_url}" width="10" height="1"></td>
